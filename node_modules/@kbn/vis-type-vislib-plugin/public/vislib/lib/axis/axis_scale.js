...
var _time_ticks = require("./time_ticks");
/*
...
class AxisScale {
  ...
  calculateExtent(data, extent) {
    ...
    let point = _d.default[extent](data);
    if (this.axisConfig.get('scale.expandLastBucket') && extent === 'max') {
      point = this.addInterval(point);
    }
    opts.push(point);
    return _d.default[extent](opts.reduce(function (opts, v) {
      if (!(0, _lodash.isNumber)(v)) v = +v;
  ...
  modByInterval(x, n) {
    ...
    const y = (0, _moment.default)(x);
    const method = n > 0 ? 'add' : 'subtract';
    (0, _lodash.times)(Math.abs(n), function () {
      y[method](interval);
    });
    return y.valueOf();
  }
  getAllPoints() {
    ...
    const chartPoints = (0, _lodash.reduce)(data, (chartPoints, chart, chartIndex) => {
  ...
  getYMin() {
    return _d.default.min(this.getAllPoints());
  }
  getYMax() {
    return _d.default.max(this.getAllPoints());
  }
  getExtents() {
    if (this.axisConfig.get('type') === 'category') {
  ...
  getD3Scale(scaleTypeArg) {
    ...
    if (this.axisConfig.isTimeDomain()) {
      return _d.default.time.scale.utc(); // allow time scale
    }

    if (this.axisConfig.isOrdinal()) return _d.default.scale.ordinal();
    if (typeof _d.default.scale[scaleType] !== 'function') {
      return this.throwCustomError(`Axis.getScaleType: ${scaleType} is not a function`);
    }
    return _d.default.scale[scaleType]();
  }
  canApplyNice() {
    const config = this.axisConfig;
  ...
  getScale(length) {
    ...
    if (this.axisConfig.isTimeDomain()) {
      this.scale.timezoneCorrectedTicks = (0, _time_ticks.timeTicks)(scale);
    }
    return this.scale;
  }
  validateScale(scale) {
