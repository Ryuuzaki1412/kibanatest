...
const reducer = (state, action) => {
  if (action.type === _types.GetStartedPageActions.ToggleProduct) {
    ...
    const {
      ...
      totalStepsLeft,
      totalActiveSteps
    } = (0, _helpers.setupActiveSections)(state.finishedSteps, activeProducts);
    return {
      ...state,
      activeProducts,
      activeSections,
  ...
  if (action.type === _types.GetStartedPageActions.RemoveFinishedStep) {
    const finishedSteps = {
      ...state.finishedSteps,
      [action.payload.cardId]: state.finishedSteps[action.payload.cardId] ? new Set([...state.finishedSteps[action.payload.cardId]]) : new Set()
    };
  ...
  if (action.type === _types.GetStartedPageActions.ToggleExpandedStep && action.payload.isStepExpanded != null) {
...
const getActiveSectionsInitialStates = ({
  activeProducts,
  finishedSteps
}) => (0, _helpers.setupActiveSections)(finishedSteps, activeProducts);
