...
function FormulaEditor({
  ...
  (0, _shared_components.useDebounceWithOptions)(() => {
    ...
    const {
      root,
      error
    } = (0, _validation.tryToParse)(text, visibleOperationsMap);
    if (error) {
      errors = [error];
    } else if (root) {
      const validationErrors = (0, _validation.runASTValidation)(root, layer, indexPattern, visibleOperationsMap, currentColumn, dateRange);
      ...
      const markers = managedColumns.flatMap(([id, column]) => {
        ...
        if (locations[id]) {
          ...
          if (def.getErrorMessage) {
            const messages = def.getErrorMessage(newLayer, id, indexPattern, dateRange, visibleOperationsMap, uiSettings.get(_public2.UI_SETTINGS.HISTOGRAM_BAR_TARGET));
            if (messages) {
  ...
  }, /*#__PURE__*/_react.default.createElement(_public.CodeEditor, (0, _extends2.default)({}, codeEditorOptions, {
    options: {
