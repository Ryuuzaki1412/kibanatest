...
function parseAndExtract(text, layer, columnId, indexPattern, operations, dateRange, label) {
  const {
    root,
    error
  } = (0, _validation.tryToParse)(text, operations);
  if (error || root == null) {
    return {
      extracted: [],
      isValid: false
  ...
  const errors = (0, _validation.runASTValidation)(root, layer, indexPattern, operations, layer.columns[columnId], dateRange);
  if (errors.length) {
    return {
      extracted: [],
      isValid: false
  ...
  /*
    ...
    */
  const extracted = extractColumns(columnId, operations, root, layer, indexPattern, _i18n.i18n.translate('xpack.lens.indexPattern.formulaPartLabel', {
