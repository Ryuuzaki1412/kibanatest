...
function isColumnInvalid(layer, columnId, indexPattern, dateRange, targetBars) {
  var _operationDefinition$;
  const column = layer.columns[columnId];
  if (!column || !indexPattern) return;
  const operationDefinition = column.operationType && _operations.operationDefinitionMap[column.operationType];
  // check also references for errors
  const referencesHaveErrors = true && 'references' in column && Boolean(getReferencesErrors(layer, column, indexPattern, dateRange, targetBars).filter(Boolean).length);
  const operationErrorMessages = operationDefinition && ((_operationDefinition$ = operationDefinition.getErrorMessage) === null || _operationDefinition$ === void 0 ? void 0 : _operationDefinition$.call(operationDefinition, layer, columnId, indexPattern, dateRange, _operations.operationDefinitionMap, targetBars));
  ...
  return operationErrorMessages && operationErrorMessages.length > 0 || referencesHaveErrors || filterHasError;
}
function getReferencesErrors(layer, column, indexPattern, dateRange, targetBars) {
  var _column$references;
  return (_column$references = column.references) === null || _column$references === void 0 ? void 0 : _column$references.map(referenceId => {
    var _layer$columns$refere, _referencedDefinition;
...
function fieldIsInvalid(layer, columnId, indexPattern) {
  var _getInvalidFieldMessa;
  const column = layer.columns[columnId];
  if (!column || !(0, _pure_utils.hasField)(column)) {
    return false;
  }
  return !!((_getInvalidFieldMessa = (0, _helpers.getInvalidFieldMessage)(layer, columnId, indexPattern)) !== null && _getInvalidFieldMessa !== void 0 && _getInvalidFieldMessa.length);
}
