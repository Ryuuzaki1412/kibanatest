  ...
  const formBasedDatasource = {
    ...
    getUserMessages(state, {
      ...
      const dimensionErrorMessages = getInvalidDimensionErrorMessages(state, layerErrorMessages, (layerId, columnId) => {
        const layer = state.layers[layerId];
        return !(0, _utils.isColumnInvalid)(layer, columnId, framePublicAPI.dataViews.indexPatterns[layer.indexPatternId], framePublicAPI.dateRange, uiSettings.get(_public.UI_SETTINGS.HISTOGRAM_BAR_TARGET));
      });
      const warningMessages = [...[...((0, _time_shift_utils.getStateTimeShiftWarningMessages)(data.datatableUtilities, state, framePublicAPI) || [])].map(longMessage => {
        const message = {
          severity: 'warning',
        ...
        return message;
      }), ...(0, _utils.getPrecisionErrorWarningMessages)(data.datatableUtilities, state, framePublicAPI, core.docLinks, setState), ...(0, _utils.getUnsupportedOperationsWarningMessage)(state, framePublicAPI, core.docLinks)];
...
function getInvalidDimensionErrorMessages(state, currentErrorMessages, isValidColumn) {
