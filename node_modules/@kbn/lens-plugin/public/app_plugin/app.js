...
function App({
  ...
  const getIsByValueMode = (0, _react.useCallback)(() => Boolean(
  // Temporarily required until the 'by value' paradigm is default.
  dashboardFeatureFlag.allowByValueEmbeddables && isLinkedToOriginatingApp && !savedObjectId), [dashboardFeatureFlag.allowByValueEmbeddables, isLinkedToOriginatingApp, savedObjectId]);
  (0, _react.useEffect)(() => {
    onAppLeave(actions => {
  ...
  }, [dashboardFeatureFlag.allowByValueEmbeddables, getOriginatingAppName, redirectToOrigin, getIsByValueMode, application, chrome, isLinkedToOriginatingApp, persistedDoc, initialContext, serverless]);
