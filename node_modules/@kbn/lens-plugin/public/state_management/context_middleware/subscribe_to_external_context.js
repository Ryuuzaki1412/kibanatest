...
function subscribeToExternalContext(data, getState, dispatch) {
  ...
  const autoRefreshSubscription = data.query.timefilter.timefilter.getAutoRefreshFetch$().pipe((0, _operators.tap)(() => {
    debounceDispatchFromExternal();
  ...
  (0, _public.waitUntilNextSessionCompletes$)(search.session).pipe((0, _operators.finalize)(done)))).subscribe();
  return () => {
