...
function toSearchResult({
  ...
  return {
    icon: iconPath ? basePath.prepend(`/plugins/enterpriseSearch/assets/source_icons/${iconPath}`) : 'logoEnterpriseSearch',
    id: serviceType,
    score,
    title: name,
    ...
    url: {
      path: url !== null && url !== void 0 ? url : `${_constants.ENTERPRISE_SEARCH_CONTENT_PLUGIN.URL}/search_indices/new_index/${serviceType === _constants.ENTERPRISE_SEARCH_CONNECTOR_CRAWLER_SERVICE_TYPE ? 'crawler' : `connector?service_type=${serviceType}`}`,
      prependBasePath: true
    }
  };
}
function getSearchResultProvider(basePath, config) {
  return {
    find: ({
      term,
      types,
      ...
      }] : []), ...(config.hasConnectors ? _searchConnectors.CONNECTOR_DEFINITIONS : []), ...(config.canDeployEntSearch ? [{
