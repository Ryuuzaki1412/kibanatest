 ...
 */
const fetchMlModels = async trainedModelsProvider => {
  ...
  const models = [_utils.ELSER_MODEL_PLACEHOLDER, _utils.ELSER_LINUX_OPTIMIZED_MODEL_PLACEHOLDER, _utils.E5_MODEL_PLACEHOLDER, _utils.E5_LINUX_OPTIMIZED_MODEL_PLACEHOLDER].filter(model => isCompatiblePromotedModelId(model.modelId));
...
const enrichModelWithDownloadStatus = async (model, trainedModelsProvider) => {
  ...
  if (modelConfigWithDefinitionStatus && modelConfigWithDefinitionStatus.count > 0) {
    ...
    model.deploymentState = modelConfigWithDefinitionStatus.trained_model_configs[0].fully_defined ? _ml.MlModelDeploymentState.NotDeployed : _ml.MlModelDeploymentState.Downloading;
  }
};
const mergeModel = (model, models) => {
