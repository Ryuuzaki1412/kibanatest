...
export API_KEY="${apiKey}"`,
  docLink: _doc_links.docLinks.restApis,
  github: {
    label: _i18n.i18n.translate('xpack.enterpriseSearch.languages.cURL.githubLink', {
      defaultMessage: 'curl'
    }),
  ...
  ingestData: ({
    ...
    return `curl -X POST "\$\{ES_URL\}/_bulk?pretty${ingestPipeline ? `&pipeline=${ingestPipeline}` : ''}" \\
  -H "Authorization: ApiKey "\$\{API_KEY\}"" \\
  -H "Content-Type: application/json" \\
  -d'
...
# example
brew install curl`,
  name: _i18n.i18n.translate('xpack.enterpriseSearch.languages.cURL', {
    defaultMessage: 'cURL'
  }),
  languageStyling: 'shell',
  testConnection: ({
    indexName = _constants.INDEX_NAME_PLACEHOLDER
  }) => `curl "\$\{ES_URL\}/${indexName}" \\
  -H "Authorization: ApiKey "\$\{API_KEY\}"" \\
  -H "Content-Type: application/json"`
};
exports.curlDefinition = curlDefinition;