...
class ExecuteReportTask {
  ...
  async _claimJob(task) {
    ...
    if (report.attempts >= maxAttempts) {
      const err = new _reportingCommon.QueueTimeoutError(`Max attempts reached (${maxAttempts}). Queue timeout reached.`);
  ...
  async _performJob(task, cancellationToken, stream) {
    ...
    const queueTimeout = (0, _reportingCommon.durationToNumber)(this.config.queue.timeout);
    return Rx.lastValueFrom(Rx.from(exportType.runTask(task.id, task.payload, cancellationToken, stream)).pipe((0, _operators.timeout)(queueTimeout)) // throw an error if a value is not emitted before timeout
    );
  }
