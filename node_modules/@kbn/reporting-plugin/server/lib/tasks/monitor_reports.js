...
class MonitorReportsTask {
  constructor(reporting, config, parentLogger) {
    ...
    this.timeout = (0, _reportingCommon.numberToDuration)(config.queue.timeout);
  }
  async getStore() {
    if (this.store) {
      return this.store;
  ...
  async init(taskManager) {
    ...
    const scheduleInterval = Math.ceil((0, _reportingCommon.numberToDuration)(this.config.queue.pollInterval).asSeconds()) + 's';
      ...
      return {
        run: async () => {
          ...
          try {
            ...
            if (![_reportingCommon.JOB_STATUS.PENDING, _reportingCommon.JOB_STATUS.PROCESSING].includes(status)) {
            ...
            if (status === _reportingCommon.JOB_STATUS.PENDING) {
            ...
            if (status === _reportingCommon.JOB_STATUS.PROCESSING) {
