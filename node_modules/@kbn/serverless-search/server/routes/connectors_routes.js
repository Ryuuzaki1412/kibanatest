...
const registerConnectorsRoutes = ({
  ...
  router.get({
    ...
    const result = await (0, _searchConnectors.fetchConnectorById)(client.asCurrentUser, request.params.connectorId);
  ...
  router.get({
    ...
    const connectors = _searchConnectors.CONNECTOR_DEFINITIONS.map(connector => ({
      ...connector,
      iconPath: connector.iconPath ? http.basePath.prepend(`/plugins/enterpriseSearch/assets/source_icons/${connector.iconPath}`) : 'logoEnterpriseSearch'
    }));
    return response.ok({
      body: {
        connectors
  ...
  router.get({
    ...
    const result = await (0, _searchConnectors.fetchSyncJobsByConnectorId)(client.asCurrentUser, request.params.connectorId, request.query.from || 0, request.query.size || 20, request.query.type || 'all');
    return response.ok({
      body: result,
      headers: {
        'content-type': 'application/json'
