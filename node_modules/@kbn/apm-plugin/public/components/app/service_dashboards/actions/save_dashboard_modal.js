...
function SaveDashboardModal({
  ...
  const [selectedDashboard, setSelectedDashboard] = (0, _react.useState)(defaultOption ? [defaultOption] : []);
  const isEditMode = !!(currentDashboard !== null && currentDashboard !== void 0 && currentDashboard.id);
  const reloadCustomDashboards = (0, _react.useCallback)(() => {
    onRefresh();
  }, [onRefresh]);
  ...
  const onClickSave = (0, _react.useCallback)(async function () {
    ...
    onClose();
  }, [selectedDashboard, notifications.toasts, serviceFiltersEnabled, onClose, reloadCustomDashboards, isEditMode, serviceName, currentDashboard, history]);
