...
exports.getEnvironmentKuery = getEnvironmentKuery;
...
function isEnvironmentDefined(environment) {
  return environment && environment !== ENVIRONMENT_NOT_DEFINED_VALUE && environment !== ENVIRONMENT_ALL_VALUE;
}
...
function getEnvironmentKuery(environment) {
  if (!isEnvironmentDefined(environment)) {
    return null;
  }
  return `${[_apm.SERVICE_ENVIRONMENT]}: ${(0, _esQuery.escapeKuery)(environment)} `;
}
