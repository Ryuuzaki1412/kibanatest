...
function isSortable(fieldName, dataView) {
  const field = dataView.getFieldByName(fieldName);
  return !!(field && field.sortable);
}
function createSortObject(sortPair, dataView) {
  if (Array.isArray(sortPair) && sortPair.length === 2 && isSortable(String(sortPair[0]), dataView)) {
    const [field, direction] = sortPair;
    return {
      [field]: direction
    };
  } else if ((0, _lodash.isPlainObject)(sortPair) && isSortable(Object.keys(sortPair)[0], dataView)) {
    return sortPair;
  }
}
 ...
 */
function getSort(sort, dataView) {
  if (Array.isArray(sort)) {
    ...
    return sort.map(sortPair => createSortObject(sortPair, dataView)).filter(sortPairObj => typeof sortPairObj === 'object');
  }
  return [];
}
 ...
 */
function getSortArray(sort, dataView) {
  return getSort(sort, dataView).reduce((acc, sortPair) => {
