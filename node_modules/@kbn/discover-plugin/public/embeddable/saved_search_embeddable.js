...
var _fetch_text_based = require("../application/main/utils/fetch_text_based");
...
class SavedSearchEmbeddable extends _public.Embeddable {
    ...
    (0, _defineProperty2.default)(this, "fetch", async () => {
      ...
      try {
        ...
        if (useTextBased && query) {
          const result = await (0, _fetch_text_based.fetchTextBased)(savedSearch.searchSource.getField('query'), dataView, this.services.data, this.services.expressions, this.services.inspector, this.abortController.signal, this.input.filters, this.input.query);
          this.updateOutput({
            ...this.getOutput(),
            loading: false
          });
          searchProps.columnTypes = result.textBasedQueryColumns ? (0, _unifiedDataTable.getTextBasedColumnTypes)(result.textBasedQueryColumns) : undefined;
  ...
  async pushContainerStateParamsToProps(searchProps, {
    ...
    searchProps.sort = this.getSort(this.input.sort || savedSearch.sort, searchProps === null || searchProps === void 0 ? void 0 : searchProps.dataView);
  ...
  renderReactComponent(domNode, searchProps) {
    ...
    const useLegacyTable = this.services.uiSettings.get(_discoverUtils.DOC_TABLE_LEGACY);
    const query = savedSearch.searchSource.getField('query');
    const props = {
      savedSearch,
      searchProps,
