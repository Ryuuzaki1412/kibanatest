...
class EditRoleMappingPage extends _react.Component {
  ...
  async loadAppData() {
    try {
      const [features, roleMapping] = await Promise.all([this.props.roleMappingsAPI.checkRoleMappingFeatures(), this.editingExistingRoleMapping() || this.cloningExistingRoleMapping() ? this.props.roleMappingsAPI.getRoleMapping(this.props.name) : Promise.resolve({
        name: '',
        enabled: true,
        metadata: {},
        role_templates: [],
      ...
      const loadState = canLoad ? 'ready' : 'permissionDenied';
      this.setState({
        loadState,
        hasCompatibleRealms,
        canUseStoredScripts,
