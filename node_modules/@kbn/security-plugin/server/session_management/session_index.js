...
exports.SessionIndex = exports.SESSION_INDEX_MAPPINGS_VERSION_META_FIELD_NAME = void 0;
 ...
 */
const SESSION_INDEX_MAPPINGS_VERSION_META_FIELD_NAME = 'security-version';

/**
 * Number of sessions to remove per batch during cleanup.
 */
exports.SESSION_INDEX_MAPPINGS_VERSION_META_FIELD_NAME = SESSION_INDEX_MAPPINGS_VERSION_META_FIELD_NAME;
...
function getSessionIndexSettings({
  ...
  return Object.freeze({
    ...
    mappings: {
      dynamic: 'strict',
      _meta: {
        [SESSION_INDEX_MAPPINGS_VERSION_META_FIELD_NAME]: SESSION_INDEX_MAPPINGS_VERSION
      },
      properties: {
        usernameHash: {
          type: 'keyword'
...
class SessionIndex {
    ...
    return this.indexInitialization = new Promise(async (resolve, reject) => {
      try {
        ...
        try {
          ...
          const errorStatusCode = (0, _errors.getErrorStatusCode)(err);
  ...
  async cleanUp() {
    ...
    try {
      for await (const sessionValues of this.getSessionValuesInBatches()) {
        const operations = sessionValues.map(({
          _id,
          _source
        }) => {
          const {
    ...
    try {
      ...
      const operations = (await this.getSessionsOutsideConcurrentSessionLimit()).map(session => {
        auditLogger.log((0, _audit.sessionCleanupConcurrentLimitEvent)({
      ...
      if (operations.length > 0) {
        ...
        for (let i = 0; i < operations.length; i += batchSize) {
          indexNeedsRefresh = (await this.bulkDeleteSessions(operations.slice(i, i + batchSize))) || indexNeedsRefresh;
        }
      }
    } catch (err) {
      logger.error(`Failed to clean up sessions that exceeded concurrent sessions limit: ${err.message}`);
      error = err;
    }
    if (indexNeedsRefresh) {
      await this.refreshSessionIndex();
    ...
    if (error) {
      logger.error(`Cleanup routine failed: ${(0, _errors.getDetailedErrorMessage)(error)}.`);
   ...
   */
  async isWithinConcurrentSessionLimit(sessionValue) {
    ...
    const maxConcurrentSessions = (_this$options$config$ = this.options.config.session.concurrentSessions) === null || _this$options$config$ === void 0 ? void 0 : _this$options$config$.maxSessions;
    if (maxConcurrentSessions == null || !sessionValue.usernameHash || sessionValue.provider.type === _authentication.AnonymousAuthenticationProvider.type) {
      return true;
    }
    let sessionsOutsideLimit;
    try {
      ...
      const searchResponse = await this.options.elasticsearchClient.search({
        ...
        query: {
          bool: {
            filter: [{
              term: {
                usernameHash: sessionValue.usernameHash
              }
            }, {
              term: {
                'provider.type': sessionValue.provider.type
              }
            }, {
              term: {
                'provider.name': sessionValue.provider.name
              }
            }]
          }
        },
      ...
      sessionsOutsideLimit = (_searchResponse$hits$ = (_searchResponse$hits = searchResponse.hits) === null || _searchResponse$hits === void 0 ? void 0 : _searchResponse$hits.hits) !== null && _searchResponse$hits$ !== void 0 ? _searchResponse$hits$ : [];
    } catch (err) {
      this.options.logger.error(`Failed to fetch user sessions to check concurrent sessions limit: ${err.message}.`);
      throw err;
    }
    ...
    const [{
      _source: sessionOutsideLimit
    }] = sessionsOutsideLimit;
    return !(sessionOutsideLimit !== null && sessionOutsideLimit !== void 0 && sessionOutsideLimit.createdAt) || sessionValue.createdAt > sessionOutsideLimit.createdAt;
  }
  ...
  async ensureSessionIndexExists() {
    ...
    this.options.logger.debug('Session index already exists. Attaching alias to the index and ensuring up-to-date mappings...');
    ...
    let indexMappingsVersion;
    try {
      var _indexMappings$this$i, _indexMappings$this$i2, _indexMappings$this$i3;
      const indexMappings = await this.options.elasticsearchClient.indices.getMapping({
        index: this.indexName
      });
      indexMappingsVersion = (_indexMappings$this$i = indexMappings[this.indexName]) === null || _indexMappings$this$i === void 0 ? void 0 : (_indexMappings$this$i2 = _indexMappings$this$i.mappings) === null || _indexMappings$this$i2 === void 0 ? void 0 : (_indexMappings$this$i3 = _indexMappings$this$i2._meta) === null || _indexMappings$this$i3 === void 0 ? void 0 : _indexMappings$this$i3[SESSION_INDEX_MAPPINGS_VERSION_META_FIELD_NAME];
    } catch (err) {
      this.options.logger.error(`Failed to retrieve session index metadata: ${err.message}`);
      throw err;
    }
    if (!indexMappingsVersion || _semver.default.lt(indexMappingsVersion, SESSION_INDEX_MAPPINGS_VERSION)) {
      var _indexMappingsVersion;
      this.options.logger.debug(`Updating session index mappings from ${(_indexMappingsVersion = indexMappingsVersion) !== null && _indexMappingsVersion !== void 0 ? _indexMappingsVersion : 'unknown'} to ${SESSION_INDEX_MAPPINGS_VERSION} version.`);
      try {
  ...
  async getSessionsOutsideConcurrentSessionLimit() {
    var _this$options$config$2, _aggResponse$aggregat, _sessionsGroupedByUse, _sessionsGroupedByUse2;
    const maxConcurrentSessions = (_this$options$config$2 = this.options.config.session.concurrentSessions) === null || _this$options$config$2 === void 0 ? void 0 : _this$options$config$2.maxSessions;
    ...
    const aggResponse = await this.options.elasticsearchClient.search({
        ...
        bool: {
          filter: [{
            ...
            bool: {
              must_not: [{
                term: {
                  'provider.type': _authentication.AnonymousAuthenticationProvider.type
                }
              }]
            }
          }]
      ...
      size: 0,
      filter_path: ['aggregations.sessions_grouped_by_user.sum_other_doc_count', 'aggregations.sessions_grouped_by_user.buckets.key', 'aggregations.sessions_grouped_by_user.buckets.doc_count'],
      track_total_hits: false
    });
    ...
    const sessionsGroupedByUser = (_aggResponse$aggregat = aggResponse.aggregations) === null || _aggResponse$aggregat === void 0 ? void 0 : _aggResponse$aggregat.sessions_grouped_by_user;
    const sessionBuckets = (_sessionsGroupedByUse = sessionsGroupedByUser === null || sessionsGroupedByUser === void 0 ? void 0 : sessionsGroupedByUser.buckets) !== null && _sessionsGroupedByUse !== void 0 ? _sessionsGroupedByUse : [];
    ...
    const ungroupedSessions = (_sessionsGroupedByUse2 = sessionsGroupedByUser === null || sessionsGroupedByUser === void 0 ? void 0 : sessionsGroupedByUser.sum_other_doc_count) !== null && _sessionsGroupedByUse2 !== void 0 ? _sessionsGroupedByUse2 : 0;
    if (ungroupedSessions > 0) {
      this.options.logger.warn(`Unable to check if remaining ${ungroupedSessions} sessions exceed the concurrent session limit. Sessions will be checked during the next cleanup job run.`);
    }
    ...
    if (skippedSessions > 0) {
      this.options.logger.warn(`Unable to fetch ${skippedSessions} sessions that exceed the concurrent session limit. Sessions will be fetched and invalidated during the next cleanup job run.`);
    }
    const {
      responses
    } = await this.options.elasticsearchClient.msearch({
      index: this.aliasName,
      searches: sessionQueries,
      filter_path: ['responses.status', 'responses.hits.hits._id']
    });
    const sessionValueDescriptors = responses.flatMap((response, index) => {
      var _response$hits$hits$m, _response$hits, _response$hits$hits;
      if ('error' in response) {
        var _ref, _getDetailedErrorMess;
        this.options.logger.error(`Failed to fetch sessions that exceed the concurrent session limit: ${(_ref = (_getDetailedErrorMess = (0, _errors.getDetailedErrorMessage)(response.error)) !== null && _getDetailedErrorMess !== void 0 ? _getDetailedErrorMess : response.error.reason) !== null && _ref !== void 0 ? _ref : response.error.type}.`);
        return [];
      }
      return (_response$hits$hits$m = (_response$hits = response.hits) === null || _response$hits === void 0 ? void 0 : (_response$hits$hits = _response$hits.hits) === null || _response$hits$hits === void 0 ? void 0 : _response$hits$hits.map(hit => ({
        sid: hit._id,
        ...sessionGroups[index]
      }))) !== null && _response$hits$hits$m !== void 0 ? _response$hits$hits$m : [];
    });
    this.options.logger.debug(`Preparing to delete ${sessionValueDescriptors.length} sessions of ${sessionBuckets.length} unique users due to exceeded concurrent sessions limit.`);
   ...
   */
  async bulkDeleteSessions(deleteOperations) {
    ...
    if (!bulkResponse.errors) {
      this.options.logger.debug(`Cleaned up ${bulkResponse.items.length} invalid or expired sessions.`);
      return true;
    }
    ...
    if (errorCount < bulkResponse.items.length) {
      this.options.logger.warn(`Failed to clean up ${errorCount} of ${bulkResponse.items.length} invalid or expired sessions. The remaining sessions were cleaned up successfully.`);
      return true;
    }
    this.options.logger.error(`Failed to clean up ${bulkResponse.items.length} invalid or expired sessions.`);
    return false;
  }
   ...
   */
  async refreshSessionIndex() {
    try {
      await this.options.elasticsearchClient.indices.refresh({
        index: this.aliasName
      });
