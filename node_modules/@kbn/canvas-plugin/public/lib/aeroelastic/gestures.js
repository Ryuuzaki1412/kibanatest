...
const metaHeld = (0, _select.select)(appleKeyboard ? e => e.metaKey : e => e.altKey)(keyFromMouse);
exports.metaHeld = metaHeld;
const optionHeld = (0, _select.select)(appleKeyboard ? e => e.altKey : e => e.ctrlKey)(keyFromMouse);
exports.optionHeld = optionHeld;
const shiftHeld = (0, _select.select)(e => e.shiftKey)(keyFromMouse);
exports.shiftHeld = shiftHeld;
const cursorPosition = (0, _select.select)(({
  cursor
}, position) => position || cursor)(gestureStatePrev, rawCursorPosition);
exports.cursorPosition = cursorPosition;
const mouseButton = (0, _select.select)(next => {
  if (!next) {
    return {
      down: false,
      up: false,
...
const mouseIsDown = (0, _select.select)(({
  mouseIsDown
}, next) => next ? next.event === 'mouseDown' : mouseIsDown)(gestureStatePrev, mouseButtonEvent);
exports.mouseIsDown = mouseIsDown;
const gestureEnd = (0, _select.select)(action => action && (action.type === 'actionEvent' || action.type === 'mouseEvent' && action.payload.event === 'mouseUp'))(primaryUpdate);
...
const mouseDowned = (0, _select.select)(state => state.buttonState === 'downed')(mouseButtonState);
exports.mouseDowned = mouseDowned;
const dragging = (0, _select.select)(state => state.buttonState === 'dragging')(mouseButtonState);
exports.dragging = dragging;
const dragVector = (0, _select.select)(({
  buttonState,
  downX,
  downY
}, {
...
exports.dragVector = dragVector;
const actionEvent = (0, _select.select)(action => action.type === 'actionEvent' ? action.payload : null)(primaryUpdate);
exports.actionEvent = actionEvent;
const gestureState = (0, _select.select)((cursor, mouseIsDown, mouseButtonState) => ({
  cursor,
  mouseIsDown,
  mouseButtonState
