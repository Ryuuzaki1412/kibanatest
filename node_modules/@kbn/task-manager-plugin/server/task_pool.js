...
class TaskPool {
  ...
  async run(tasks, attempt = 1) {
    ...
    if (attempt > MAX_RUN_ATTEMPTS) {
      const stats = [`availableWorkers: ${availableWorkers}`, `tasksToRun: ${tasksToRun.length}`, `leftOverTasks: ${leftOverTasks.length}`, `maxWorkers: ${this.maxWorkers}`, `occupiedWorkers: ${this.occupiedWorkers}`, `workerLoad: ${this.workerLoad}`].join(', ');
      this.logger.warn(`task pool run attempts exceeded ${MAX_RUN_ATTEMPTS}; assuming ran out of capacity; ${stats}`);
  ...
  handleMarkAsRunning(taskRunner) {
    taskRunner.run().catch(err => {
      ...
      const errorLogLine = `Task ${taskRunner.toString()} failed in attempt to run: ${err.message || err.error.message}`;
