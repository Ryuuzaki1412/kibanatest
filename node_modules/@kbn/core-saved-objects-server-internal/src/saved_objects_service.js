...
class SavedObjectsService {
  ...
  async setup(setupDeps) {
    ...
    return {
      status$: (0, _status.calculateStatus$)(skipMigration ? (0, _rxjs.of)({
        status: 'completed'
      }) : this.migrator$.pipe((0, _operators.switchMap)(migrator => migrator.getStatus$())), elasticsearch.status$),
  ...
  async start({
    ...
    const migrator = this.createMigrator(this.config.migration, elasticsearch.client.asInternalUser, docLinks, waitForMigrationCompletion, node, elasticsearch.getCapabilities());
    ...
    if (skipMigrations) {
      ...
      try {
        ...
        await (0, _rxjs.firstValueFrom)(this.setupDeps.elasticsearch.esNodesCompatibility$.pipe((0, _operators.filter)(nodes => nodes.isCompatible)));
      } catch (e) {
