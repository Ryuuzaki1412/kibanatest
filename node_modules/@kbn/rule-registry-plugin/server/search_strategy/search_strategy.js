...
const ruleRegistrySearchStrategyProvider = (data, alerting, logger, security, spaces) => {
  ...
  return {
    search: (request, options, deps) => {
      ...
      return (0, _rxjs.from)(getAsync(request.featureIds)).pipe((0, _operators.mergeMap)(({
        space,
        authzFilter,
        authorizedRuleTypes
      }) => {
        var _request$query, _request$query$bool, _request$query2, _request$query2$bool, _request$query3, _request$query3$bool, _request$query4, _request$query4$bool, _request$sort, _request$query5, _request$query6, _request$fields;
        const indices = alerting.getAlertIndicesAlias(authorizedRuleTypes.map(art => art.id), space === null || space === void 0 ? void 0 : space.id);
        if (indices.length === 0) {
