...
sampleProbability = 1, emitError, abortSignal) => {
  ...
  for (const [index, settledPromise] of settledPromises.entries()) {
    ...
    const overallResult = randomSamplerWrapper.unwrap(resp.aggregations).change_point_p_value;
    for (const bucket of overallResult.buckets) {
      const pValue = Math.exp(-bucket.score);
      if (typeof pValue === 'number' && pValue < _constants.LOG_RATE_ANALYSIS_SETTINGS.P_VALUE_THRESHOLD) {
        result.push({
