...
function routeHandlerFactory(version, license, logger, coreStart, usageCounter) {
  return async (context, request, response) => {
    ...
    (0, _track_route_usage.trackAIOpsRouteUsage)(`POST ${_api.AIOPS_API_ENDPOINT.LOG_RATE_ANALYSIS}`, headers[_constants.AIOPS_TELEMETRY_ID.AIOPS_ANALYSIS_RUN_ORIGIN], usageCounter);
    if (!license.isActivePlatinumLicense) {
      return response.forbidden();
    }
    const client = (await context.core).elasticsearch.client.asCurrentUser;
    const executionContext = (0, _mlRouteUtils.createExecutionContext)(coreStart, _common.PLUGIN_ID, request.route.path);
