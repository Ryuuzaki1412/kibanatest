...
const UserProfilesSelectable = ({
  ...
  const updateDisplayedOptions = () => {
    ...
    setDisplayedOptions(values => {
      ...
      const selectedOptionsToAdd = selectedOptions ? selectedOptions.filter(profile => !nextOptions.find(option => isMatchingOption(option, profile))).map(option => toSelectableOption(option, nullOptionLabel)) : [];

      // Get any newly added default options
      const defaultOptionsToAdd = defaultOptions ? defaultOptions.filter(profile => !nextOptions.find(option => isMatchingOption(option, profile)) && !selectedOptionsToAdd.find(option => isMatchingOption(option, profile))).map(option => toSelectableOption(option, nullOptionLabel)) : [];
    ...
    renderOption: (option, searchValue) => {
      if (option.user) {
        return /*#__PURE__*/_react.default.createElement(_eui.EuiFlexGroup, {
          ...
          search: searchValue
        }, option.label)), option.user.email && option.user.email !== option.label ? /*#__PURE__*/_react.default.createElement(_eui.EuiFlexItem, {
          grow: false
...
function toSelectableOption(userProfile, nullOptionLabel) {
  if (userProfile) {
    return {
      key: userProfile.uid,
      label: (0, _user_profile.getUserDisplayName)(userProfile.user),
      data: userProfile
    };
  }
  return {
