...
function GraphWorkspace(options) {
  ...
  this.runLayout = function () {
    ...
    this.force = _d.default.layout.force().nodes(visibleNodes).links(effectiveEdges).friction(0.8).linkDistance(100).charge(-1500).gravity(0.15).theta(0.99).alpha(0.5).size([800, 600]).on('tick', function () {
  ...
  this.mergeGraph = function (newData) {
    ...
    dedupedNodes.forEach(dedupedNode => {
      ...
      const node = {
        ...
        icon: (0, _style_choices.getIcon)(dedupedNode.icon),
        data: dedupedNode
      };
      ...
      data.vertices.forEach(node => {
        targetFields.some(fieldDef => {
          if (node.field === fieldDef.name) {
            ...
            node.icon = (0, _style_choices.getIcon)(fieldDef.icon);
  ...
  this.getLikeThisButNotThisQuery = function (startNodes) {
    ...
    blocklistedNodes.forEach(blocklistedNode => {
      addTermToFieldList(excludeNodesByField, blocklistedNode.data.field, blocklistedNode.data.term);
    });
