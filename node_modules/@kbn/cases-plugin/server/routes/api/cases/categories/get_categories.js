...
const getCategoriesRoute = (0, _create_cases_route.createCasesRoute)({
  method: 'get',
  path: _constants.INTERNAL_GET_CASE_CATEGORIES_URL,
  handler: async ({
    context,
    request,
    response
    ...
    try {
      ...
      const res = await client.cases.getCategories(options);
      return response.ok({
        body: res
      });
    } catch (error) {
