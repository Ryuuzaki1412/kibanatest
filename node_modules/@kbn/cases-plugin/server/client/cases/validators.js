...
function validateCustomFieldTypesInRequest({
  ...
  let invalidCustomFieldKeys = [];
  const validCustomFields = (0, _lodash.intersectionWith)(customFieldsConfiguration, requestCustomFields, (requiredVal, requestedVal) => requiredVal.key === requestedVal.key && requiredVal.type === requestedVal.type);
  if (requestCustomFields.length !== validCustomFields.length) {
    invalidCustomFieldKeys = (0, _lodash.differenceWith)(requestCustomFields, validCustomFields, (requiredVal, requestedVal) => requiredVal.key === requestedVal.key).map(e => e.key);
  }
  if (invalidCustomFieldKeys.length) {
    throw _boom.default.badRequest(`The following custom fields have the wrong type in the request: ${invalidCustomFieldKeys}`);
  }
}
...
const validateRequiredCustomFields = ({
  ...
  const missingRequiredCustomFields = (0, _lodash.differenceWith)(requiredCustomFields, requestCustomFields !== null && requestCustomFields !== void 0 ? requestCustomFields : [], (requiredVal, requestedVal) => requiredVal.key === requestedVal.key).map(e => `"${e.label}"`);
  ...
  if (missingRequiredCustomFields.length) {
    throw _boom.default.badRequest(`Missing required custom fields: ${missingRequiredCustomFields.join(', ')}`);
  }
};
