...
const validateConvertFilterToKueryNode = (allowedTypes, filter, indexMapping) => {
  if (filter && indexMapping) {
    ...
    if (validationFilterKuery.length === 0) {
      throw _coreSavedObjectsServer.SavedObjectsErrorHelpers.createBadRequestError('If we have a filter options defined, we should always have validationFilterKuery defined too');
    }
    if (validationFilterKuery.some(obj => obj.error != null)) {
      throw _coreSavedObjectsServer.SavedObjectsErrorHelpers.createBadRequestError(validationFilterKuery.filter(obj => obj.error != null).map(obj => obj.error).join('\n'));
    }
