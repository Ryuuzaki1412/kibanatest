...
const performBulkGet = async ({
  ...
  const result = {
    saved_objects: expectedBulkGetResults.map(expectedResult => {
      ...
      // @ts-expect-error MultiGetHit._source is optional
      const document = (0, _utils2.getSavedObjectFromSource)(registry, type, id, doc, {
        migrationVersionCompatibility
      });
  ...
  return encryptionHelper.optionallyDecryptAndRedactBulkResult(result, authorizationResult === null || authorizationResult === void 0 ? void 0 : authorizationResult.typeMap);
};
