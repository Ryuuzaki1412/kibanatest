...
class AlertingAuthorization {
  ...
  async augmentRuleTypesWithAuthorization(ruleTypes, operations, authorizationEntity, featuresIds) {
    ...
    if (this.authorization && this.shouldCheckAuthorization()) {
      ...
      const addLegacyConsumerPrivileges = legacyConsumer => legacyConsumer === _types.ALERTS_FEATURE_ID || (0, _lodash.isEmpty)(featuresIds);
...
const getValidConsumer = ({
  ...
  legacyConsumer,
  producer
}) => legacyConsumer === _types.ALERTS_FEATURE_ID || validLegacyConsumers.includes(legacyConsumer) ? producer : legacyConsumer;
