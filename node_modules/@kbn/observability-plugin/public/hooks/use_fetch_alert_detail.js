...
const fetchAlert = async (params, abortController, http) => {
  const {
    id,
    ruleType
  } = params;
  try {
    const response = await http.get(_constants.BASE_RAC_ALERTS_API_PATH, {
      query: {
        id
      },
    ...
    if (response !== undefined) {
      return (0, _parse_alert.parseAlert)(ruleType)(response);
    }
  } catch (error) {
