...
class KQLCustomTransformGenerator extends _.TransformGenerator {
  getTransformParams(slo) {
    if (!_sloSchema.kqlCustomIndicatorSchema.is(slo.indicator)) {
  ...
  buildAggregations(slo, indicator) {
    const numerator = (0, _.getElastichsearchQueryOrThrow)(indicator.params.good);
    const denominator = (0, _.getElastichsearchQueryOrThrow)(indicator.params.total);
    return {
      'slo.numerator': {
        filter: numerator
      },
