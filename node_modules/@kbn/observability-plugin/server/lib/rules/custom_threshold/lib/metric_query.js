...
const calculateCurrentTimeframe = (metricParams, timeframe) => ({
  ...timeframe,
  start: (0, _moment.default)(timeframe.end).subtract(metricParams.timeSize, metricParams.timeUnit).valueOf()
});
exports.calculateCurrentTimeframe = calculateCurrentTimeframe;
const createBaseFilters = (metricParams, timeframe, timeFieldName, filterQuery) => {
  const rangeFilters = [{
    range: {
      [timeFieldName]: {
  ...
  const parsedFilterQuery = (0, _get_parsed_filtered_query.getParsedFilterQuery)(filterQuery);
  return [...rangeFilters, ...parsedFilterQuery];
};
exports.createBaseFilters = createBaseFilters;
const getElasticsearchMetricQuery = (metricParams, timeframe, timeFieldName, compositeSize, alertOnGroupDisappear, lastPeriodEnd, groupBy, filterQuery, afterKey, fieldsExisted) => {
  ...
  const metricAggregations = (0, _create_custom_metrics_aggregations.createCustomMetricsAggregations)('aggregatedValue', metricParams.metrics, metricParams.equation);
