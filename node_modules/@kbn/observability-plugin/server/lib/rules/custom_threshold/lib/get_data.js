...
const getData = async (esClient, params, index, timeFieldName, groupBy, filterQuery, compositeSize, alertOnGroupDisappear, timeframe, logger, lastPeriodEnd, previousResults = {}, afterKey) => {
  const handleResponse = (aggs, previous, successfulShards) => {
    ...
    if (aggs.groupings) {
      ...
      if (nextAfterKey) {
        return getData(esClient, params, index, timeFieldName, groupBy, filterQuery, compositeSize, alertOnGroupDisappear, timeframe, logger, lastPeriodEnd, previous, nextAfterKey);
      }
      return previous;
    }
  ...
  const request = {
    ...
    body: (0, _metric_query.getElasticsearchMetricQuery)(params, timeframe, timeFieldName, compositeSize, alertOnGroupDisappear, lastPeriodEnd, groupBy, filterQuery, afterKey, fieldsExisted)
  };
