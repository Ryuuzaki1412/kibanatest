...
const evaluateRule = async (esClient, params, dataView, timeFieldName, compositeSize, alertOnGroupDisappear, logger, timeframe, lastPeriodEnd, missingGroups = []) => {
  ...
  return Promise.all(criteria.map(async criterion => {
    ...
    const intervalAsSeconds = (0, _utils.getIntervalInSeconds)(interval);
    const intervalAsMS = intervalAsSeconds * 1000;
    const calculatedTimerange = (0, _create_timerange.createTimerange)(intervalAsMS, timeframe, lastPeriodEnd);
    const currentValues = await (0, _get_data.getData)(esClient, criterion, dataView, timeFieldName, groupBy, searchConfiguration.query.query, compositeSize, alertOnGroupDisappear, calculatedTimerange, logger, lastPeriodEnd);
    const verifiedMissingGroups = await (0, _check_missing_group.checkMissingGroups)(esClient, criterion, dataView, timeFieldName, groupBy, searchConfiguration.query.query, logger, calculatedTimerange, missingGroups);
