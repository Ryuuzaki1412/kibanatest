...
const keywordScoreWeighting = 0.8;
exports.keywordScoreWeighting = keywordScoreWeighting;
const getAppResults = (term, apps) => {
  return apps
  // Unroll all deepLinks, only if there is a search term
  .flatMap(app => {
    var _app$keywords;
    return term.length > 0 ? flattenDeepLinks(app) : app.searchable ? [{
      id: app.id,
      app,
      path: app.appRoute,
      subLinkTitles: [],
...
const flattenDeepLinks = (app, deepLink) => {
  ...
  return [...(deepLink.path && deepLink.searchable ? [{
    ...deepLink,
