...
const getConflictBtn = (fieldName, conflictDescriptions, openModal, theme) => {
  const onClick = () => {
    const overlayRef = openModal((0, _public.toMountPoint)(getConflictModalContent({
      closeFn: () => {
        overlayRef.close();
      },
      fieldName,
...
class Table extends _react.PureComponent {
  ...
  renderFieldType(type, field) {
    ...
    return /*#__PURE__*/_react.default.createElement("span", null, type === 'conflict' && conflictDescription ? '' : type, field.conflictDescriptions ? getConflictBtn(field.name, field.conflictDescriptions, this.props.openModal, this.props.theme) : '');
  }
  render() {
    const {
      items,
