...
const RuleForm = ({
  ...
  (0, _react.useEffect)(() => {
    ...
    const getAvailableRuleTypes = ruleTypesResult => ruleTypeRegistry.list().reduce((arr, ruleTypeRegistryItem) => {
      ...
      ruleType,
      validConsumers
    })).filter(item => rule.consumer === _common.ALERTS_FEATURE_ID ? !item.ruleTypeModel.requiresAppContext : item.ruleType.producer === rule.consumer);
  ...
  const authorizedConsumers = (0, _react.useMemo)(() => {
    ...
    if (rule.consumer !== _common.ALERTS_FEATURE_ID) {
      return [];
    }
  ...
  const hasFieldsForAAD = (0, _react.useMemo)(() => {
    ...
    if (_constants.MULTI_CONSUMER_RULE_TYPE_IDS.includes((_rule$ruleTypeId = rule === null || rule === void 0 ? void 0 : rule.ruleTypeId) !== null && _rule$ruleTypeId !== void 0 ? _rule$ruleTypeId : '')) {
      return (validConsumers || _rule_form_consumer_selection.VALID_CONSUMERS).includes(selectedConsumer) && hasAlertHasData;
    }
    return hasAlertHasData;
  }, [rule === null || rule === void 0 ? void 0 : rule.ruleTypeId, selectedConsumer, selectedRuleType, validConsumers]);
