...
function convertMessageToMarkdownCodeBlock(message) {
  ...
  if (!message.name) {
    ...
    const content = message.role !== _common.MessageRole.Assistant && message.content ? (0, _safe_json_parse.safeJsonParse)(message.content) : message.content;
...
function getTimelineItemsfromConversation({
  ...
  const messagesWithoutSystem = messages.filter(message => message.message.role !== _common.MessageRole.System);
  const items = [{
    id: (0, _uuid.v4)(),
    actions: {
      canCopy: false,
    ...
    switch (role) {
      case _common.MessageRole.User:
        ...
        if (message.message.name && prevFunctionCall) {
          ...
          if ((prevFunctionCall === null || prevFunctionCall === void 0 ? void 0 : prevFunctionCall.trigger) === _common.MessageRole.Assistant) {
          ...
          if (startedFrom === 'contextualInsight') {
            const firstUserMessageIndex = messagesWithoutSystem.findIndex(el => el.message.role === _common.MessageRole.User);
            display.collapsed = index === firstUserMessageIndex;
          }
        }
        break;
      case _common.MessageRole.Assistant:
  ...
  const isLoading = chatState === _use_chat.ChatState.Loading;
  let lastMessage = (0, _lodash.last)(items);
  const isNaturalLanguageOnlyAnswerFromAssistant = ((_lastMessage = lastMessage) === null || _lastMessage === void 0 ? void 0 : _lastMessage.message.message.role) === _common.MessageRole.Assistant && !((_lastMessage$message$ = lastMessage.message.message.function_call) !== null && _lastMessage$message$ !== void 0 && _lastMessage$message$.name);
  ...
  if (addLoadingPlaceholder) {
    items.push({
      ...
      content: '',
      currentUser,
      loading: chatState === _use_chat.ChatState.Loading,
      role: _common.MessageRole.Assistant,
      title: '',
      message: {
        '@timestamp': new Date().toISOString(),
