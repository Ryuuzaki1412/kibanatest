...
const createInventoryMetricThresholdExecutor = libs => libs.metricsRules.createLifecycleRuleExecutor(async ({
  ...
  if (!params.filterQuery && params.filterQueryText) {
    try {
      ...
      const indexedStartedAt = (_getAlertStartedDate = getAlertStartedDate(_utils.UNGROUPED_FACTORY_KEY)) !== null && _getAlertStartedDate !== void 0 ? _getAlertStartedDate : startedAt.toISOString();
  ...
  for (const group of inventoryItems) {
    ...
    if (reason) {
      var _additionalContext$ta, _getAlertStartedDate2;
      const actionGroupId = nextState === _metrics.AlertStates.WARNING ? WARNING_ACTIONS_ID : FIRED_ACTIONS_ID;
      ...
      const evaluationValues = results.reduce((acc, result) => {
        acc.push(result[group].currentValue);
        return acc;
      }, []);
      const alert = alertFactory(group, reason, actionGroupId, additionalContext, evaluationValues);
      const indexedStartedAt = (_getAlertStartedDate2 = getAlertStartedDate(group)) !== null && _getAlertStartedDate2 !== void 0 ? _getAlertStartedDate2 : startedAt.toISOString();
  ...
  for (const alert of recoveredAlerts) {
    ...
    const indexedStartedAt = (_getAlertStartedDate3 = getAlertStartedDate(recoveredAlertId)) !== null && _getAlertStartedDate3 !== void 0 ? _getAlertStartedDate3 : startedAt.toISOString();
