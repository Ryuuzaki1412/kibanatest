...
var _create_custom_metrics_aggregations = require("../../../lib/create_custom_metrics_aggregations");
/*
...
const convertMetricToMetricsAPIMetric = (metric, index) => {
  ...
  if (metric.aggregation === 'rate' && metric.field) {
    return {
      id,
      aggregations: (0, _common.networkTraffic)(id, metric.field)
    };
  }
  ...
  if (metric.aggregation === 'custom' && metric.custom_metrics) {
    const customMetricAggregations = (0, _create_custom_metrics_aggregations.createCustomMetricsAggregations)(id, metric.custom_metrics, metric.equation);
    if (!(0, _lodash.isEmpty)(customMetricAggregations)) {
      return {
        id,
