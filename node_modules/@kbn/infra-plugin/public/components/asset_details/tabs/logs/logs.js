...
const Logs = () => {
  ...
  const filter = (0, _react.useMemo)(() => {
    const query = [`${(0, _common2.findInventoryFields)(asset.type).id}: "${asset.name}"`, ...(textQueryDebounced !== '' ? [textQueryDebounced] : [])].join(' and ');
    return {
      language: 'kuery',
      query
    };
  ...
  }, [locators.nodeLogsLocator, asset.name, asset.type, state.startTimestamp, textQueryDebounced, logView]);
