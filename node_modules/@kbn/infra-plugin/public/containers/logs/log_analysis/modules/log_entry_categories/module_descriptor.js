...
const moduleName = _i18n.i18n.translate('xpack.infra.logs.analysis.logEntryCategoriesModuleName', {
  defaultMessage: 'Categorization'
});
const moduleDescription = _i18n.i18n.translate('xpack.infra.logs.analysis.logEntryCategoriesModuleDescription', {
  defaultMessage: 'Use Machine Learning to automatically categorize log messages.'
});
const getJobIds = (spaceId, logViewId, idFormat) => _log_analysis.logEntryCategoriesJobTypes.reduce((accumulatedJobIds, jobType) => ({
  ...accumulatedJobIds,
...
const setUpModule = async (start, end, datasetFilter, {
  ...
  const jobOverrides = [{
    job_id: _log_analysis.logEntryCategoriesJobType,
    analysis_config: {
  ...
  const query = {
    bool: {
      filter: [...(datasetFilter.type === 'includeSome' ? [{
        terms: {
...
const cleanUpModule = async (spaceId, logViewId, idFormat, fetch) => {
  return await (0, _log_analysis_cleanup.cleanUpJobsAndDatafeeds)(spaceId, logViewId, idFormat, _log_analysis.logEntryCategoriesJobTypes, fetch);
};
...
const logEntryCategoriesModule = {
  moduleId,
  moduleName,
  moduleDescription,
  jobTypes: _log_analysis.logEntryCategoriesJobTypes,
