...
const getDataFromFieldsHits = (fields, prependField, prependFieldCategory) => Object.keys(fields).reduce((accumulator, field) => {
  ...
  if (isRuleParametersFieldOrSubfield(field, prependField)) {
    nestedFields = Array.isArray(item) ? item.reduce((acc, i) => {
      acc.push(getDataFromFieldsHits(i, dotField, fieldCategory));
      return acc;
    }, []).flat() : getDataFromFieldsHits(item, dotField, fieldCategory);
  } else {
    nestedFields = Array.isArray(item) ? item.reduce((acc, i) => {
      acc.push(getDataFromFieldsHits(i, dotField, fieldCategory));
      return acc;
    }, []).flat() : getDataFromFieldsHits(item, prependField, fieldCategory);
  }
