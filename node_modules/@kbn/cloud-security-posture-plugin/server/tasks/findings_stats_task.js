...
exports.aggregateLatestFindings = exports.CSPM_FINDINGS_STATS_INTERVAL = void 0;
...
const CSPM_FINDINGS_STATS_INTERVAL = 5;
exports.CSPM_FINDINGS_STATS_INTERVAL = CSPM_FINDINGS_STATS_INTERVAL;
...
const getFindingsScoresDocIndexingPromises = (esClient, scoresByPolicyTemplatesBuckets) => scoresByPolicyTemplatesBuckets.map(policyTemplateTrend => {
  ...
  const benchmarkStats = Object.fromEntries(policyTemplateTrend.score_by_benchmark_id.buckets.map(benchmarkIdBucket => {
    ...
    const benchmarkVersions = Object.fromEntries(benchmarkIdBucket.benchmark_versions.buckets.map(benchmarkVersionBucket => {
      const benchmarkVersion = (0, _mapping_field_util.toBenchmarkMappingFieldKey)(benchmarkVersionBucket.key);
...
const aggregateLatestFindings = async (esClient, logger) => {
  try {
    var _scoreIndexQueryResul;
    const startAggTime = performance.now();
    const scoreIndexQueryResult = await esClient.search(getScoreQuery());
    ...
    const scoresByPolicyTemplatesBuckets = ((_scoreIndexQueryResul = scoreIndexQueryResult.aggregations) === null || _scoreIndexQueryResul === void 0 ? void 0 : _scoreIndexQueryResul.score_by_policy_template.buckets) || [];

    // iterating over the buckets and return promises which will index a modified document into the scores index
    const findingsScoresDocIndexingPromises = getFindingsScoresDocIndexingPromises(esClient, scoresByPolicyTemplatesBuckets);
    ...
    await Promise.all([...findingsScoresDocIndexingPromises, vulnStatsTrendDocIndexingPromises].filter(Boolean));
