  ...
  await Promise.all(preconfiguredFleetProxies.map(async preconfiguredFleetProxy => {
    ...
    if (isCreate) {
      ...
      if (fleetServerHosts.some(host => host.is_default) || outputs.some(output => output.is_default || output.is_default_monitoring)) {
        ...
        await (0, _pMap.default)(outputs, output => _agent_policy.agentPolicyService.bumpAllAgentPoliciesForOutput(soClient, esClient, output.id), {
          concurrency: 20
        });
        await (0, _pMap.default)(fleetServerHosts, fleetServerHost => _agent_policy.agentPolicyService.bumpAllAgentPoliciesForFleetServerHosts(soClient, esClient, fleetServerHost.id), {
          concurrency: 20
        });
      }
    }
