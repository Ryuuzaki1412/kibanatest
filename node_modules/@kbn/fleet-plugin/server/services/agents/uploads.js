...
async function getAgentUploads(esClient, agentId) {
  const getFile = async actionId => {
    try {
      ...
      if (fileResponse.hits.hits.length === 0) {
        _app_context.appContextService.getLogger().debug(`No matches for action_id ${actionId} and agent_id ${agentId}`);
        return;
      }
      return {
  ...
  for (const action of actions) {
    ...
    const status = (_file$Status = file === null || file === void 0 ? void 0 : file.Status) !== null && _file$Status !== void 0 ? _file$Status : action.error ? 'FAILED' : isActionExpired ? 'EXPIRED' : 'IN_PROGRESS';
    const result = {
