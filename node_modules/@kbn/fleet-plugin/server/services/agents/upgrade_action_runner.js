...
async function upgradeBatch(soClient, esClient, givenAgents, outgoingErrors, options) {
  ...
  const upgradeableResults = await Promise.allSettled(agentsToCheckUpgradeable.map(async agent => {
    ...
    const isNotAllowed = (0, _services.getRecentUpgradeInfoForAgent)(agent).hasBeenUpgradedRecently || !options.force && !(0, _services.isAgentUpgradeable)(agent, latestAgentVersion, options.version);
    if (isNotAllowed) {
...
exports.MINIMUM_EXECUTION_DURATION_SECONDS = MINIMUM_EXECUTION_DURATION_SECONDS;
const EXPIRATION_DURATION_SECONDS = 60 * 60 * 24 * 30; // 1 month
exports.EXPIRATION_DURATION_SECONDS = EXPIRATION_DURATION_SECONDS;
const getRollingUpgradeOptions = (startTime, upgradeDurationSeconds) => {
  ...
  if (upgradeDurationSeconds) {
    ...
    return {
      ...
      expiration: (0, _moment.default)(startTime !== null && startTime !== void 0 ? startTime : now).add(upgradeDurationSeconds <= MINIMUM_EXECUTION_DURATION_SECONDS ? minExecutionDuration * 2 : upgradeDurationSeconds, 'seconds').toISOString()
    };
  }
  ...
  if (startTime && !upgradeDurationSeconds) {
    return {
      ...
      expiration: (0, _moment.default)(startTime).add(EXPIRATION_DURATION_SECONDS, 'seconds').toISOString()
    };
  } else {
