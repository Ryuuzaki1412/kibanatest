...
async function getFullAgentPolicy(soClient, id, options) {
  ...
  const inputs = await (0, _.storedPackagePoliciesToAgentInputs)(agentPolicy.package_policies, packageInfoCache, getOutputIdForAgentPolicy(dataOutput));
...
function transformOutputToFullPolicyOutput(output, proxy, standalone = false) {
  ...
  if (type === _constants.outputType.Kafka) {
    ...
    kafkaData = {
      ...
      topics: (topics !== null && topics !== void 0 ? topics : []).map(topic => {
        const {
          topic: topicName,
          ...rest
        } = topic;
