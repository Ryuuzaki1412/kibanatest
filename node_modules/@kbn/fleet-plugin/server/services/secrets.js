...
async function isSecretStorageEnabled(esClient, soClient) {
  ...
  if (await (0, _fleet_server.allFleetServerVersionsAreAtLeast)(esClient, soClient, _constants.SECRETS_MINIMUM_FLEET_SERVER_VERSION)) {
...
async function isOutputSecretStorageEnabled(esClient, soClient) {
  ...
  // otherwise check if we have the minimum fleet server version and enable secrets if so
  if (await (0, _fleet_server.allFleetServerVersionsAreAtLeast)(esClient, soClient, _constants.OUTPUT_SECRETS_MINIMUM_FLEET_SERVER_VERSION)) {
