...
function getTemplate({
  ...
  template.composed_of = [...esBaseComponents, ...(template.composed_of || []), _constants.FLEET_GLOBALS_COMPONENT_TEMPLATE_NAME, ...((_appContextService$ge = _.appContextService.getConfig()) !== null && _appContextService$ge !== void 0 && _appContextService$ge.agentIdVerificationEnabled ? [_constants.FLEET_AGENT_ID_VERIFY_COMPONENT_TEMPLATE_NAME] : [])];
  return template;
}
const getBaseEsComponents = (type, isIndexModeTimeSeries) => {
  if (type === 'metrics') {
    ...
    return [_constants.STACK_COMPONENT_TEMPLATE_LOGS_SETTINGS];
  }
  return [];
};
...
const updateExistingDataStream = async ({
  ...
  let lifecycle;
  try {
    const simulateResult = await (0, _retry.retryTransientEsErrors)(async () => esClient.indices.simulateTemplate({
