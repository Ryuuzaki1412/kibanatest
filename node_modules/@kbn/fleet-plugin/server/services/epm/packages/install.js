...
exports.installIndexTemplatesAndPipelines = installIndexTemplatesAndPipelines;
exports.installPackage = installPackage;
exports.isPackageInstalled = isPackageInstalled;
exports.isPackageVersionOrLaterInstalled = isPackageVersionOrLaterInstalled;
exports.optimisticallyAddEsAssetReferences = void 0;
exports.restartInstallation = restartInstallation;
exports.updateVersion = exports.updateInstallStatusToFailed = exports.updateEsAssetReferences = exports.saveKibanaAssetsRefs = void 0;
...
var _utils2 = require("./custom_integrations/utils");
var _constants4 = require("./custom_integrations/constants");
var _custom_integrations = require("./custom_integrations");
var _cache = require("./custom_integrations/assets/cache");
var _utils3 = require("./custom_integrations/assets/dataset/utils");
...
async function installCustomPackage(args) {
  ...
  const packageInfo = {
    ...
    name: pkgName,
    title: (0, _utils2.convertStringToTitle)(pkgName),
    description: (0, _utils2.generateDescription)(datasets.map(dataset => dataset.name)),
    ...
    type: 'integration',
    data_streams: (0, _utils3.generateDatastreamEntries)(datasets, pkgName)
  };
  ...
  const paths = (0, _cache.cacheAssets)(assets, pkgName, _constants4.INITIAL_VERSION);
  return await installPackageCommon({
    pkgName,
...
const optimisticallyAddEsAssetReferences = async (savedObjectsClient, pkgName, assetsToAdd) => {
  const addEsAssets = async () => {
    var _so$attributes$instal;
    // TODO: Should this be replaced by a `get()` call from epm/get.ts?
    const so = await savedObjectsClient.get(_constants3.PACKAGES_SAVED_OBJECT_TYPE, pkgName);
    _audit_logging.auditLoggingService.writeCustomSoAuditLog({
      action: 'get',
      id: pkgName,
      savedObjectType: _constants3.PACKAGES_SAVED_OBJECT_TYPE
    ...
    } = await savedObjectsClient.update(_constants3.PACKAGES_SAVED_OBJECT_TYPE, pkgName, {
      installed_es: deduplicatedAssets
    }, {
      version: so.version
    });
...
async function installIndexTemplatesAndPipelines({
  installedPkg,
  paths,
  packageInfo,
  esReferences,
   ...
   */
  const experimentalDataStreamFeatures = (_installedPkg$experim = installedPkg === null || installedPkg === void 0 ? void 0 : installedPkg.experimental_data_stream_features) !== null && _installedPkg$experim !== void 0 ? _installedPkg$experim : [];
  const preparedIngestPipelines = (0, _ingest_pipeline.prepareToInstallPipelines)(packageInfo, paths, onlyForDataStreams);
  const preparedIndexTemplates = (0, _install2.prepareToInstallTemplates)(packageInfo, paths, esReferences, experimentalDataStreamFeatures, onlyForDataStreams);
  ...
  if (onlyForDataStreams) {
    ...
    newEsReferences = await optimisticallyAddEsAssetReferences(savedObjectsClient, packageInfo.name, [...preparedIngestPipelines.assetsToAdd, ...preparedIndexTemplates.assetsToAdd]);
  } else {
    newEsReferences = await updateEsAssetReferences(savedObjectsClient, packageInfo.name, esReferences, {
...
async function installAssetsForInputPackagePolicy(opts) {
  ...
  const paths = await (0, _archive.getArchiveFilelist)(pkgInfo);
  if (!paths) throw new _errors.PackageInvalidArchiveError(`No paths found for ${pkgInfo.name}`);
  ...
  if (existingDataStreams.length) {
    ...
    if (existingDataStreamsAreFromDifferentPackage && !force) {
      ...
      logger.info(`Data stream ${dataStream.name} already exists, skipping index template creation for ${packagePolicy.id}`);
      return;
    }
  }
  ...
  const installedPkg = await (0, _2.getInstallation)({
    savedObjectsClient: soClient,
    pkgName: pkgInfo.name,
    logger
  });
  if (!installedPkg) throw new _errors.PackageNotFoundError(`Error while creating index templates: unable to find installed package ${pkgInfo.name}`);
