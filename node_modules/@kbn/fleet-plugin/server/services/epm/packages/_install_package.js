...
async function _installPackage({
  ...
  try {
    ...
    if (!isILMPoliciesDisabled) {
      esReferences = await (0, _utils.withPackageSpan)('Install ILM policies', () => (0, _install.installILMPolicy)(packageInfo, paths, esClient, savedObjectsClient, logger, esReferences));
      ({
        esReferences
      } = await (0, _utils.withPackageSpan)('Install Data Stream ILM policies', () => (0, _install4.installIlmForDataStream)(packageInfo, paths, esClient, savedObjectsClient, logger, esReferences)));
    }
    ...
    if (packageInfo.type === 'integration') {
      ...
      } = await (0, _install5.installIndexTemplatesAndPipelines)({
    ...
    const updatedPackage = await (0, _utils.withPackageSpan)('Update install status', () => {
