...
const installPackageByUploadHandler = async (context, request, response) => {
  var _appContextService$ge3, _request$query4, _request$query5;
  const coreContext = await context.core;
  const fleetContext = await context.fleet;
  const savedObjectsClient = fleetContext.internalSoClient;
  const esClient = coreContext.elasticsearch.client.asInternalUser;
...
const reauthorizeTransformsHandler = async (context, request, response) => {
  ...
  try {
    ...
    const user = await ((_appContextService$ge4 = _services.appContextService.getSecurity()) === null || _appContextService$ge4 === void 0 ? void 0 : _appContextService$ge4.authc.getCurrentUser(request));
    if (user) {
...
const getInputsHandler = async (context, request, response) => {
  ...
  try {
    ...
    let body;
    if (format === 'json') {
      body = await (0, _packages.getTemplateInputs)(soClient, pkgName, pkgVersion, 'json');
    } else if (format === 'yml' || format === 'yaml') {
      body = await (0, _packages.getTemplateInputs)(soClient, pkgName, pkgVersion, 'yml');
    }
    return response.ok({
      body
    });
