...
const ViewErrors = ({
  ...
  var _coreStart$cloud, _action$latestErrors2;
  const coreStart = (0, _hooks.useStartServices)();
  const isLogsUIAvailable = !((_coreStart$cloud = coreStart.cloud) !== null && _coreStart$cloud !== void 0 && _coreStart$cloud.isServerlessEnabled);
  const getLogsButton = (agentId, timestamp, viewInLogs) => {
    const startTime = (0, _moment.default)(timestamp).subtract(5, 'm').toISOString();
    const endTime = (0, _moment.default)(timestamp).add(5, 'm').toISOString();
