...
function usePackagePolicyWithRelatedData(packagePolicyId, options) {
  ...
  const savePackagePolicy = async () => {
    ...
    } = await (0, _prepare_input_pkg_policy_dataset.prepareInputPackagePolicyDataset)(packagePolicy);
  ...
  (0, _react.useEffect)(() => {
    const getData = async () => {
      ...
      try {
        ...
        } = await (0, _hooks.sendGetOneAgentPolicy)(packagePolicyData.item.policy_id);
        if (agentPolicyError) {
          throw agentPolicyError;
        }
        if (agentPolicyData !== null && agentPolicyData !== void 0 && agentPolicyData.item) {
        ...
        if (basePolicy) {
          ...
          const newPackagePolicy = {
            ...
            inputs: baseInputs.map(input => {
              ...
              let basePolicyInputVars = isUpgradeScenario && ((_basePolicy$inputs$fi = basePolicy.inputs.find(i => i.type === input.type && i.policy_template === input.policy_template)) === null || _basePolicy$inputs$fi === void 0 ? void 0 : _basePolicy$inputs$fi.vars);
              let newInputVars = inputVars;
              if (basePolicyInputVars && inputVars) {
                // merging vars from dry run with updated ones
                basePolicyInputVars = Object.keys(inputVars).reduce((acc, curr) => ({
                  ...acc,
                  [curr]: basePolicyInputVars[curr]
                }), {});
                newInputVars = {
                  ...inputVars,
                  ...basePolicyInputVars
