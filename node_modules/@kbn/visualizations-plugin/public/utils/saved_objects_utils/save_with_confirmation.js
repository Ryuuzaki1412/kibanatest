...
async function saveWithConfirmation(source, savedObject, options, services) {
  ...
  try {
    ...
    if ((0, _lodash.get)(err, 'res.status') === 409) {
      ...
      return (0, _confirm_modal_promise.confirmModalPromise)(confirmMessage, title, confirmButtonText, overlays).then(() => _content_management.visualizationsClient.create({
        data: source,
        options: {
          overwrite: true,
          ...options
