...
exports.containersRoutes = containersRoutes;
...
function containersRoutes({
  router,
  assetClient
}) {
  const validate = (0, _ioTsUtils.createRouteValidationFunction)(_types_api.getContainerAssetsQueryOptionsRT);
  router.get({
    ...
    try {
      ...
      (0, _debug_log.debug)('Error while looking up CONTAINER asset records', error);
      ...
      return res.customError({
        statusCode: 500,
        body: {
          message: 'Error while looking up container asset records - ' + `${error}`
        }
      });
    }
  });
