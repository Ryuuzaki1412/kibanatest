...
exports.categoryFieldTypes = void 0;
...
const categoryFieldTypes = [_fieldTypes.ES_FIELD_TYPES.TEXT, _fieldTypes.ES_FIELD_TYPES.KEYWORD, _fieldTypes.ES_FIELD_TYPES.IP, _fieldTypes.ES_FIELD_TYPES.VERSION];

// cross reference fields and aggs.
// fields contain a list of aggs that are compatible, and vice versa.
exports.categoryFieldTypes = categoryFieldTypes;
function combineFieldsAndAggs(fields, aggs, rollupFields) {
  ...
  aggs.forEach(a => {
    if (a.type === _mlAnomalyUtils.METRIC_AGG_TYPE && a.fields !== undefined) {
      switch (a.id) {
        case _mlAnomalyUtils.ML_JOB_AGGREGATION.LAT_LONG:
          geoFields.forEach(f => mix(f, a));
          break;
        case _mlAnomalyUtils.ML_JOB_AGGREGATION.INFO_CONTENT:
        case _mlAnomalyUtils.ML_JOB_AGGREGATION.HIGH_INFO_CONTENT:
        case _mlAnomalyUtils.ML_JOB_AGGREGATION.LOW_INFO_CONTENT:
          textFields.forEach(f => mix(f, a));
        case _mlAnomalyUtils.ML_JOB_AGGREGATION.DISTINCT_COUNT:
        case _mlAnomalyUtils.ML_JOB_AGGREGATION.HIGH_DISTINCT_COUNT:
        case _mlAnomalyUtils.ML_JOB_AGGREGATION.LOW_DISTINCT_COUNT:
...
function filterFields(fields) {
  return fields.filter(f => f.aggs && (f.aggs.length > 0 || f.aggs.length === 0 && f.type === _fieldTypes.ES_FIELD_TYPES.DATE));
}
...
function getKeywordFields(fields) {
  return fields.filter(f => f.type === _fieldTypes.ES_FIELD_TYPES.KEYWORD || f.type === _fieldTypes.ES_FIELD_TYPES.VERSION);
}
function getTextFields(fields) {
  return fields.filter(f => f.type === _fieldTypes.ES_FIELD_TYPES.TEXT);
}
function getIpFields(fields) {
  return fields.filter(f => f.type === _fieldTypes.ES_FIELD_TYPES.IP);
}
function getNumericalFields(fields) {
  return fields.filter(f => f.type === _fieldTypes.ES_FIELD_TYPES.LONG || f.type === _fieldTypes.ES_FIELD_TYPES.UNSIGNED_LONG || f.type === _fieldTypes.ES_FIELD_TYPES.INTEGER || f.type === _fieldTypes.ES_FIELD_TYPES.SHORT || f.type === _fieldTypes.ES_FIELD_TYPES.BYTE || f.type === _fieldTypes.ES_FIELD_TYPES.DOUBLE || f.type === _fieldTypes.ES_FIELD_TYPES.FLOAT || f.type === _fieldTypes.ES_FIELD_TYPES.HALF_FLOAT || f.type === _fieldTypes.ES_FIELD_TYPES.SCALED_FLOAT);
}
function getGeoFields(fields) {
  return fields.filter(f => f.type === _fieldTypes.ES_FIELD_TYPES.GEO_POINT || f.type === _fieldTypes.ES_FIELD_TYPES.GEO_SHAPE);
}
...
function sortFields(fields) {
  ...
  let eventRate;
  if (fields[0].id === _mlAnomalyUtils.EVENT_RATE_FIELD_ID) {
