...
var _url_utils = require("../util/url_utils");
var _ml_api_service = require("./ml_api_service");
var _toast_notification_service = require("./toast_notification_service");
/*
...
class JobService {
  constructor() {
    (0, _defineProperty2.default)(this, "loadJobsWrapper", () => {
      return this.loadJobs().then(function (resp) {
        return resp;
  ...
  loadJobs() {
    return new Promise((resolve, reject) => {
      jobs = [];
      datafeedIds = {};
      _ml_api_service.ml.getJobs().then(resp => {
        jobs = resp.jobs;

        // load jobs stats
        _ml_api_service.ml.getJobStats().then(statsResp => {
          ...
          for (let i = 0; i < jobs.length; i++) {
            ...
            for (let j = 0; j < statsResp.jobs.length; j++) {
              ...
              if (job.job_id === jobStats.job_id) {
                ...
                job.data_counts = (0, _lodash.cloneDeep)(jobStats.data_counts);
                job.model_size_stats = (0, _lodash.cloneDeep)(jobStats.model_size_stats);
                if (jobStats.node) {
                  job.node = (0, _lodash.cloneDeep)(jobStats.node);
                }
      ...
      function error(err) {
        ...
        (0, _toast_notification_service.getToastNotificationService)().displayErrorToast(err);
        reject({
          jobs,
          err
        });
    ...
    return new Promise((resolve, reject) => {
      _ml_api_service.ml.getJobs({
        ...
        if (resp.jobs && resp.jobs.length) {
          ...
          // load jobs stats
          _ml_api_service.ml.getJobStats({
            jobId
          }).then(statsResp => {
            // merge jobs stats into jobs
            for (let j = 0; j < statsResp.jobs.length; j++) {
              if (newJob.job_id === statsResp.jobs[j].job_id) {
                ...
                newJob.data_counts = (0, _lodash.cloneDeep)(statsJob.data_counts);
                newJob.model_size_stats = (0, _lodash.cloneDeep)(statsJob.model_size_stats);
                if (newJob.node) {
                  newJob.node = (0, _lodash.cloneDeep)(statsJob.node);
                }
  ...
  loadDatafeeds(datafeedId) {
    return new Promise((resolve, reject) => {
      const sId = datafeedId !== undefined ? {
        datafeed_id: datafeedId
      } : undefined;
      _ml_api_service.ml.getDatafeeds(sId).then(resp => {
        const datafeeds = resp.datafeeds;

        // load datafeeds stats
        _ml_api_service.ml.getDatafeedStats().then(statsResp => {
  ...
  updateSingleJobDatafeedState(jobId) {
    return new Promise((resolve, reject) => {
      const datafeedId = this.getDatafeedId(jobId);
      _ml_api_service.ml.getDatafeedStats({
        datafeedId
      }).then(resp => {
  ...
  saveNewJob(job) {
    ...
    return _ml_api_service.ml.addJob({
      jobId: job.job_id,
      job
    }).then(func).catch(func);
  }
  cloneDatafeed(datafeed) {
    const tempDatafeed = (0, _lodash.cloneDeep)(datafeed);
  ...
  getJob(jobId) {
    const job = (0, _lodash.find)(jobs, j => {
      return j.job_id === jobId;
    });
    return job;
  }
  openJob(jobId) {
    return _ml_api_service.ml.openJob({
      jobId
    });
  }
  closeJob(jobId) {
    return _ml_api_service.ml.closeJob({
      jobId
    });
  }
  ...
  deleteJobs(jIds, deleteUserAnnotations) {
    return _ml_api_service.ml.jobs.deleteJobs(jIds, deleteUserAnnotations);
  }
  closeJobs(jIds) {
    return _ml_api_service.ml.jobs.closeJobs(jIds);
  }
  resetJobs(jIds, deleteUserAnnotations) {
    return _ml_api_service.ml.jobs.resetJobs(jIds, deleteUserAnnotations);
  }
  validateDetector(detector) {
    return new Promise((resolve, reject) => {
      if (detector) {
        _ml_api_service.ml.validateDetector({
          detector
        }).then(resp => {
          resolve(resp);
        }).catch(resp => {
  ...
  getJobGroups() {
    ...
    (0, _lodash.each)(tempGroups, (js, id) => {
      groups.push({
        id,
        jobs: js
      });
  ...
  async getJobAndGroupIds() {
    try {
      return await _ml_api_service.ml.jobs.getAllJobAndGroupIds();
    } catch (error) {
      return {
        jobIds: [],
        groupIds: []
...
function processBasicJobInfo(localJobService, jobsList) {
  ...
  const jobsListCopy = (0, _lodash.cloneDeep)(jobsList);
  (0, _lodash.each)(jobsListCopy, jobObj => {
    const analysisConfig = jobObj.analysis_config;
    const bucketSpan = (0, _parse_interval.parseInterval)(analysisConfig.bucket_span);
    const job = {
      id: jobObj.job_id,
    ...
    job.detectors = (0, _lodash.get)(analysisConfig, 'detectors', []);
    detectorsByJob[job.id] = job.detectors;
    if (jobObj.custom_settings !== undefined && jobObj.custom_settings.custom_urls !== undefined) {
      job.customUrls = [];
      (0, _lodash.each)(jobObj.custom_settings.custom_urls, url => {
        if (url.url_name !== undefined && url.url_value !== undefined && (0, _url_utils.isWebUrl)(url.url_value)) {
...
function createResultsUrlForJobs(jobsList, resultsPage, userTimeRange) {
  ...
  if ((0, _mlDateUtils.validateTimeRange)(userTimeRange)) {
    from = userTimeRange.from;
    to = userTimeRange.to;
    // if both pass datemath's checks but are not technically absolute dates, use 'quick'
    // e.g. "now-15m" "now+1d"
    const fromFieldAValidDate = (0, _moment.default)(userTimeRange.from).isValid();
    const toFieldAValidDate = (0, _moment.default)(userTimeRange.to).isValid();
  ...
  const fromString = (0, _moment.default)(from).format(_mlDateUtils.TIME_FORMAT); // Defaults to 'now' if 'from' is undefined
  const toString = (0, _moment.default)(to).format(_mlDateUtils.TIME_FORMAT); // Defaults to 'now' if 'to' is undefined
...
const mlJobService = new JobService();
exports.mlJobService = mlJobService;