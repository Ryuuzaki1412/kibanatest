...
exports.calculateAggregationInterval = calculateAggregationInterval;
exports.calculateDefaultFocusRange = calculateDefaultFocusRange;
exports.calculateInitialFocusRange = calculateInitialFocusRange;
exports.createTimeSeriesJobData = createTimeSeriesJobData;
exports.findChartPointForAnomalyTime = findChartPointForAnomalyTime;
exports.findChartPointForScheduledEvent = findChartPointForScheduledEvent;
exports.findNearestChartPointToTime = findNearestChartPointToTime;
exports.getAutoZoomDuration = getAutoZoomDuration;
...
function createTimeSeriesJobData(jobs) {
  const singleTimeSeriesJobs = jobs.filter(_job_utils.isTimeSeriesViewJob);
...
function findNearestChartPointToTime(chartData, time) {
  ...
  if (chartPoint === undefined) {
    ...
    let foundItem;
    for (let i = 0; i < chartData.length; i++) {
      var _chartData$i, _chartData$i$date;
      const itemTime = (_chartData$i = chartData[i]) === null || _chartData$i === void 0 ? void 0 : (_chartData$i$date = _chartData$i.date) === null || _chartData$i$date === void 0 ? void 0 : _chartData$i$date.getTime();
      if (itemTime > time) {
        var _previousItem$date;
        const item = chartData[i];
        const previousItem = chartData[i - 1];
        const diff1 = Math.abs(time - (previousItem === null || previousItem === void 0 ? void 0 : (_previousItem$date = previousItem.date) === null || _previousItem$date === void 0 ? void 0 : _previousItem$date.getTime()));
...
function calculateAggregationInterval(bounds, bucketsTarget, jobs, selectedJob) {
  ...
  const buckets = (0, _time_buckets.getTimeBucketsFromCache)();
  ...
  const bucketSpanSeconds = (0, _lodash.find)(jobs, {
...
function calculateInitialFocusRange(zoomState, contextAggregationInterval, bounds) {
  if (zoomState !== undefined) {
    ...
    const zoomFrom = (0, _momentTimezone.default)(zoomState.from, 'YYYY-MM-DDTHH:mm:ss.SSSZ', true);
    const zoomTo = (0, _momentTimezone.default)(zoomState.to, 'YYYY-MM-DDTHH:mm:ss.SSSZ', true);
    const searchBounds = (0, _time_buckets.getBoundsRoundedToInterval)(bounds, contextAggregationInterval, true);
...
function getAutoZoomDuration(jobs, selectedJob) {
  ...
  let autoZoomDuration = bucketSpanSeconds * 1000 * (_timeseriesexplorer_constants.CHARTS_POINT_TARGET - 1);
  ...
  const max = (0, _momentTimezone.default)(nowMs);
  const min = (0, _momentTimezone.default)(nowMs - autoZoomDuration);
  buckets.setBounds({
    min,
    max
  });
