...
const modelMemoryEstimatorProvider = (jobCreator, jobValidator) => {
  ...
  return {
    ...
    get updates$() {
      return (0, _rxjs.combineLatest)([jobCreator.wizardInitialized$.pipe((0, _operators.skipWhile)(wizardInitialized => wizardInitialized === false)), modelMemoryCheck$]).pipe((0, _operators.map)(([, payload]) => payload),
      // delay the request, making sure the validation is completed
      (0, _operators.debounceTime)(_job_validator.VALIDATION_DELAY_MS + 100),
      // clone the object to compare payloads and proceed further only
      // if the configuration has been changed
      (0, _operators.map)(_lodash.cloneDeep), (0, _operators.distinctUntilChanged)(_lodash.isEqual),
      // don't call the endpoint with invalid payload
      (0, _operators.filter)(() => jobValidator.isModelMemoryEstimationPayloadValid), (0, _operators.switchMap)(payload => {
        return _ml_api_service.ml.calculateModelMemoryLimit$(payload).pipe((0, _operators.pluck)('modelMemoryLimit'), (0, _operators.catchError)(error => {
...
const useModelMemoryEstimator = (jobCreator, jobValidator, jobCreatorUpdate, jobCreatorUpdated) => {
  ...
  (0, _react.useEffect)(() => {
    ...
    subscription.add(modelMemoryEstimator.updates$.pipe((0, _operators.startWith)(jobCreator.modelMemoryLimit), (0, _operators.pairwise)()).subscribe(([previousEstimation, currentEstimation]) => {
