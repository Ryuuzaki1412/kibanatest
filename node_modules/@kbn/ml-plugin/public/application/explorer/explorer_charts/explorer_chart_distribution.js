...
class ExplorerChartDistribution extends _react.default.Component {
  ...
  renderChart() {
    ...
    const fieldFormat = _field_format_service.mlFieldFormatService.getFieldFormat(config.jobId, config.detectorIndex);
    let vizWidth = 0;
    ...
    function init({
      ...
      const $el = (0, _jquery.default)('.ml-explorer-chart');
      ...
      const svg = chartElement.append('svg').classed('ml-explorer-chart-svg', true).attr('width', svgWidth).attr('height', svgHeight);
      ...
      if (chartType === _explorer_constants.CHART_TYPE.POPULATION_DISTRIBUTION) {
        ...
        lineChartYScale = _d.default.scale.linear().range([chartHeight, 0]).domain([yScaleDomainMin < 0 ? yScaleDomainMin : 0, yScaleDomainMax]).nice();
      } else if (chartType === _explorer_constants.CHART_TYPE.EVENT_DISTRIBUTION) {
        // avoid overflowing the border of the highlighted area
        const rowMargin = 5;
        lineChartYScale = _d.default.scale.ordinal().rangePoints([rowMargin, chartHeight - rowMargin]).domain(scaleCategories);
      } else {
    ...
    function drawSyncedCursorLine(lineChartGroup) {
      var _chartTheme$crosshair, _chartTheme$crosshair2, _chartTheme$crosshair3;
      lineChartGroup.append('rect').attr('x', 0).attr('y', 0).attr('height', chartHeight).attr('width', vizWidth).on('mouseout', function () {
        onPointerUpdate({
      ...
      const cursorData = cursor && cursor.type === 'Over' && cursor.x >= config.plotEarliest && cursor.x <= config.plotLatest ? [cursor.x] : [];
      const cursorMouseLine = lineChartGroup.append('g').attr('class', 'ml-anomaly-chart-cursor').selectAll('.ml-anomaly-chart-cursor-line').data(cursorData);
      ...
      .style('stroke', `${(_chartTheme$crosshair = chartTheme.crosshair.line.stroke) !== null && _chartTheme$crosshair !== void 0 ? _chartTheme$crosshair : 'black'}`).style('stroke-width', `${(_chartTheme$crosshair2 = chartTheme.crosshair.line.strokeWidth) !== null && _chartTheme$crosshair2 !== void 0 ? _chartTheme$crosshair2 : '1'}px`).style('stroke-dasharray', (_chartTheme$crosshair3 = chartTheme.crosshair.line.dash) !== null && _chartTheme$crosshair3 !== void 0 ? _chartTheme$crosshair3 : '4,4');
    ...
    function drawRareChartAxes() {
      ...
      const xAxis = _d.default.svg.axis().scale(lineChartXScale).orient('bottom').innerTickSize(-chartHeight).outerTickSize(0).tickPadding(10).tickFormat(d => (0, _moment.default)(d).format(xAxisTickFormat));
      ...
      const gAxis = axes.append('g').attr('class', 'x axis').attr('transform', 'translate(0,' + chartHeight + ')').call(xAxis);
    ...
    function drawRareChartMarkers(data) {
      ...
      dots.enter().append('circle').attr('r', LINE_CHART_ANOMALY_RADIUS)
      // Don't use an arrow function since we need access to `this`.
      ...
      dots.attr('cx', d => lineChartXScale(d.date)).attr('cy', d => lineChartYScale(d[CHART_Y_ATTRIBUTE])).attr('class', d => {
        ...
        if (d.anomalyScore !== undefined && Number(d.anomalyScore) >= severity) {
          ...
          markerClass += (0, _mlAnomalyUtils.getSeverityWithLow)(d.anomalyScore).id;
        }
        return markerClass;
      });
      ...
      scheduledEventMarkers.enter().append('rect').attr('width', LINE_CHART_ANOMALY_RADIUS * 2).attr('height', SCHEDULED_EVENT_MARKER_HEIGHT).attr('class', 'scheduled-event-marker').attr('rx', 1).attr('ry', 1);

      // Update all markers to new positions.
      scheduledEventMarkers.attr('x', d => lineChartXScale(d.date) - LINE_CHART_ANOMALY_RADIUS).attr('y', d => lineChartYScale(d[CHART_Y_ATTRIBUTE]) - SCHEDULED_EVENT_MARKER_HEIGHT / 2);
    }
...
(0, _defineProperty2.default)(ExplorerChartDistribution, "propTypes", {
  ...
  severity: _propTypes.default.number,
