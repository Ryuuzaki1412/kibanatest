...
const turnOffPolicyProtectionsIfNotSupported = async (esClient, fleetServices, appFeaturesService, logger) => {
  const log = logger.get('endpoint', 'policyProtections');
  if (appFeaturesService.isEnabled(_keys.AppFeatureSecurityKey.endpointPolicyProtections)) {
    log.info(`App feature [${_keys.AppFeatureSecurityKey.endpointPolicyProtections}] is enabled. Nothing to do!`);
    return;
  }
  log.info(`App feature [${_keys.AppFeatureSecurityKey.endpointPolicyProtections}] is disabled. Checking endpoint integration policies for compliance`);
  const {
    packagePolicy,
    internalSoClient,
    endpointPolicyKuery
  ...
  do {
    ...
    for (const item of items) {
      ...
      if (!isOnlyCollectingEvents) {
        ...
        integrationPolicy.inputs[0].config.policy.value = (0, _policy_config_helpers.ensureOnlyEventCollectionIsAllowed)(policySettings);
        updates.push({
