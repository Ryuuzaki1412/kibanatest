...
const getActionDetailsList = async ({
  ...
  const actionDetails = normalizedActionRequests.map(action => {
    ...
    const {
      ...
      agentState,
      outputs
    } = (0, _utils.getActionCompletionInfo)(action, matchedResponses);
    const {
      isExpired,
      status
    ...
    const actionRecord = {
      ...
      completedAt,
      wasSuccessful,
      errors: (_action$error = action.error) !== null && _action$error !== void 0 && _action$error.message ? [action.error.message] : errors,
      agentState,
      isExpired,
      status,
