...
const createUserRiskEnrichments = async ({
  ...
  return (0, _create_single_field_match_enrichment.createSingleFieldMatchEnrichment)({
    ...
    createEnrichmentFunction: enrichment => event => {
      ...
      if (riskLevel) {
        (0, _saferLodashSet.set)(newEvent, '_source.user.risk.calculated_level', riskLevel);
      }
      if (riskScore) {
        (0, _saferLodashSet.set)(newEvent, '_source.user.risk.calculated_score_norm', riskScore);
      }
      return newEvent;
    }
  });
