...
const bulkCreateRulesRoute = (router, ml, logger) => {
  router.versioned.post({
    ...
    validate: {
      request: {
        body: (0, _route_validation.buildRouteValidationWithZod)(_rule_management.BulkCreateRulesRequestBody)
      }
    }
    ...
    try {
      ...
      const rules = await Promise.all(ruleDefinitions.filter(rule => rule.rule_id == null || !dupes.includes(rule.rule_id)).map(async payloadRule => {
        ...
        try {
          ...
          return (0, _validate.transformValidateBulkError)(createdRule.params.ruleId, createdRule);
        } catch (err) {
