...
const bulkUpdateRulesRoute = (router, ml, logger) => {
  router.versioned.put({
    ...
    validate: {
      request: {
        body: (0, _route_validation.buildRouteValidationWithZod)(_rule_management.BulkUpdateRulesRequestBody)
      }
    }
    ...
    try {
      ...
      const rules = await Promise.all(request.body.map(async payloadRule => {
        ...
        try {
          ...
          if (rule != null) {
            return (0, _validate.transformValidateBulkError)(rule.id, rule);
          } else {
