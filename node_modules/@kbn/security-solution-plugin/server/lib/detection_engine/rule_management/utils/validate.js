...
exports.validateResponseActionsPermissions = exports.transformValidateBulkError = exports.transformValidate = void 0;
...
const transformValidate = rule => {
  const transformed = (0, _utils3.transform)(rule);
  return _rule_schema.RuleResponse.parse(transformed);
};
exports.transformValidate = transformValidate;
const transformValidateBulkError = (ruleId, rule) => {
  if ((0, _rule_schema2.hasValidRuleType)(rule)) {
    const transformed = (0, _rule_converters.internalRuleToAPIResponse)(rule);
    const result = _rule_schema.RuleResponse.safeParse(transformed);
    if (!result.success) {
...
const validateResponseActionsPermissions = async (securitySolution, ruleUpdate, existingRule) => {
  ...
  symmetricDifference.forEach(action => {
    ...
    const authzPropName = _constants.RESPONSE_CONSOLE_ACTION_COMMANDS_TO_REQUIRED_AUTHZ[_constants.RESPONSE_ACTION_API_COMMANDS_TO_CONSOLE_COMMAND_MAP[action.params.command]];
