...
const previewRulesRoute = async (router, config, ml, security, ruleOptions, securityRuleTypeOptions, previewRuleDataClient, getStartServices, logger) => {
  router.versioned.post({
    ...
    validate: {
      request: {
        body: (0, _route_validation.buildRouteValidationWithZod)(_detection_engine.PreviewRulesSchema)
      }
    }
    ...
    try {
      ...
      switch (previewRuleParams.type) {
        ...
        case 'esql':
          ...
          const esqlAlertType = previewRuleTypeWrapper((0, _rule_types.createEsqlAlertType)(ruleOptions));
          await runExecutors(esqlAlertType.executor, esqlAlertType.id, esqlAlertType.name, previewRuleParams, () => true, {
            create: _alert_instance_factory_stub.alertInstanceFactoryStub,
            alertLimit: {
              getValue: () => 1000,
              setLimitReached: () => {}
