...
const getQueryOrder = sort => {
  if (sort.field === _search_strategy.NetworkDnsFields.queryCount) {
    return {
      _count: {
        order: sort.direction
      }
    };
  } else if (sort.field === _search_strategy.NetworkDnsFields.dnsName) {
    return {
      _key: {
        order: sort.direction
      }
    };
  } else if (sort.field === _search_strategy.NetworkDnsFields.uniqueDomains) {
    return {
      unique_domains: {
        order: sort.direction
      }
    };
  } else if (sort.field === _search_strategy.NetworkDnsFields.dnsBytesIn) {
    return {
      dns_bytes_in: {
        order: sort.direction
      }
    };
  } else if (sort.field === _search_strategy.NetworkDnsFields.dnsBytesOut) {
    return {
      dns_bytes_out: {
        order: sort.direction
      }
