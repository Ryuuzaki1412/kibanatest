...
const AnomaliesNetworkTableComponent = ({
  startDate,
  endDate,
  skip,
  ip,
  ...
  const {
    toggleStatus,
    setToggleStatus
  } = (0, _query_toggle.useQueryToggle)(`AnomaliesNetwork-${flowTarget}`);
  ...
  const getAnomaliesUserTableFilterQuerySelector = (0, _react.useMemo)(() => _store.networkSelectors.networkAnomaliesJobIdFilterSelector(), []);
  ...
  const onSelectJobId = (0, _react.useCallback)(newSelection => {
    dispatch(_store.networkActions.updateNetworkAnomaliesJobIdFilter({
      jobIds: newSelection,
      networkType: type
    }));
  }, [dispatch, type]);
  const getAnomaliesNetworkTableIntervalQuerySelector = (0, _react.useMemo)(() => _store.networkSelectors.networkAnomaliesIntervalSelector(), []);
  const selectedInterval = (0, _use_selector.useDeepEqualSelector)(state => getAnomaliesNetworkTableIntervalQuerySelector(state, type));
  const onSelectInterval = (0, _react.useCallback)(newInterval => {
    dispatch(_store.networkActions.updateNetworkAnomaliesInterval({
  ...
  const networks = (0, _convert_anomalies_to_network.convertAnomaliesToNetwork)(tableData, jobNameById, ip);
  const columns = (0, _get_anomalies_network_table_columns.getAnomaliesNetworkTableColumnsCurated)(type, startDate, endDate, flowTarget);
  const pagination = {
    initialPageIndex: 0,
    initialPageSize: 10,
...
const AnomaliesNetworkTable = /*#__PURE__*/_react.default.memo(AnomaliesNetworkTableComponent);
