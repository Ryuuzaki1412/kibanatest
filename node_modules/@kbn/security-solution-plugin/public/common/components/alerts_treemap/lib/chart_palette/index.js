...
var _data = require("../../../../../detections/components/rules/step_about_rule/data");
/*
 ...
 */
exports.RISK_SCORE_STEPS = RISK_SCORE_STEPS;
const getRiskScorePalette = steps => Array(steps).fill(0).map((_, i) => {
  if (i >= _data.RISK_SCORE_CRITICAL) {
    return _data.RISK_COLOR_CRITICAL;
  } else if (i >= _data.RISK_SCORE_HIGH) {
    return _data.RISK_COLOR_HIGH;
  } else if (i >= _data.RISK_SCORE_MEDIUM) {
    return _data.RISK_COLOR_MEDIUM;
  } else {
    return _data.RISK_COLOR_LOW;
  }
});
