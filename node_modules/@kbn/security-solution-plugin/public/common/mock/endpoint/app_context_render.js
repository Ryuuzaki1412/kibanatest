...
const createAppRootMockRenderer = () => {
  ...
  const {
    storage
  } = (0, _.createSecuritySolutionStorageMock)();
  ...
  const store = (0, _store.createStore)(_.mockGlobalState, storeReducer, _test_providers.kibanaObservable, storage,
...
const createCoreStartMock = history => {
  ...
  const linkPaths = getLinksPaths(_app_links.links);
...
const getLinksPaths = appLinks => {
  return appLinks.reduce((result, link) => {
    if (link.path) {
