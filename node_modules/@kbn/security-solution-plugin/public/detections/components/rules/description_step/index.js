...
const getDescriptionItem = (field, label, data, filterManager, license, indexPatterns) => {
  if (field === 'queryBar') {
    ...
    const ruleCanHaveGroupByFields = (0, _utils.isQueryRule)(ruleType);
    if (!ruleCanHaveGroupByFields) {
      return [];
    }
    const values = (0, _fp.get)(field, data);
    return (0, _helpers.buildAlertSuppressionDescription)(label, values);
  } else if (field === 'groupByRadioSelection') {
    return [];
  } else if (field === 'groupByDuration') {
    const ruleType = (0, _fp.get)('ruleType', data);
    const ruleCanHaveDuration = (0, _utils.isQueryRule)(ruleType) || (0, _utils.isThresholdRule)(ruleType);
    ...
    if (showDuration) {
      ...
      return (0, _helpers.buildAlertSuppressionWindowDescription)(label, value, (0, _fp.get)('groupByRadioSelection', data));
    } else {
      return [];
    }
  } else if (field === 'suppressionMissingFields') {
    const ruleType = (0, _fp.get)('ruleType', data);
    const ruleCanHaveSuppressionMissingFields = (0, _utils.isQueryRule)(ruleType);
    if (!ruleCanHaveSuppressionMissingFields) {
      return [];
    }
    if ((0, _fp.get)('groupByFields', data).length > 0) {
      const value = (0, _fp.get)(field, data);
      return (0, _helpers.buildAlertSuppressionMissingFieldsDescription)(label, value);
    } else {
      return [];
    }
    ...
    return (0, _helpers.buildQueryBarDescription)({
      ...
      savedId,
      indexPatterns,
      queryLabel: i18n.THREAT_QUERY_LABEL
    });
