"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.RuleResponseOsqueryAction = exports.RuleResponseEndpointAction = exports.RuleResponseAction = exports.ResponseActionTypesEnum = exports.ResponseActionTypes = exports.ResponseAction = exports.OsqueryResponseAction = exports.OsqueryQuery = exports.OsqueryParamsCamelCase = exports.OsqueryParams = exports.EndpointResponseAction = exports.EndpointParams = exports.EcsMapping = void 0;
var _zod = require("zod");
/*
 * Copyright Elasticsearch B.V. and/or licensed to Elasticsearch B.V. under one
 * or more contributor license agreements. Licensed under the Elastic License
 * 2.0; you may not use this file except in compliance with the Elastic License
 * 2.0.
 */

/*
 * NOTICE: Do not edit this file manually.
 * This file is automatically generated by the OpenAPI Generator, @kbn/openapi-generator.
 */

const ResponseActionTypes = _zod.z.enum(['.osquery', '.endpoint']);
exports.ResponseActionTypes = ResponseActionTypes;
const ResponseActionTypesEnum = ResponseActionTypes.enum;
exports.ResponseActionTypesEnum = ResponseActionTypesEnum;
const EcsMapping = _zod.z.object({}).catchall(_zod.z.object({
  field: _zod.z.string().optional(),
  value: _zod.z.union([_zod.z.string(), _zod.z.array(_zod.z.string())]).optional()
}));
exports.EcsMapping = EcsMapping;
const OsqueryQuery = _zod.z.object({
  /**
   * Query ID
   */
  id: _zod.z.string(),
  /**
   * Query to execute
   */
  query: _zod.z.string(),
  ecs_mapping: EcsMapping.optional(),
  /**
   * Query version
   */
  version: _zod.z.string().optional(),
  platform: _zod.z.string().optional(),
  removed: _zod.z.boolean().optional(),
  snapshot: _zod.z.boolean().optional()
});
exports.OsqueryQuery = OsqueryQuery;
const OsqueryParams = _zod.z.object({
  query: _zod.z.string().optional(),
  ecs_mapping: EcsMapping.optional(),
  queries: _zod.z.array(OsqueryQuery).optional(),
  pack_id: _zod.z.string().optional(),
  saved_query_id: _zod.z.string().optional(),
  timeout: _zod.z.number().optional()
});
exports.OsqueryParams = OsqueryParams;
const OsqueryParamsCamelCase = _zod.z.object({
  query: _zod.z.string().optional(),
  ecsMapping: EcsMapping.optional(),
  queries: _zod.z.array(OsqueryQuery).optional(),
  packId: _zod.z.string().optional(),
  savedQueryId: _zod.z.string().optional(),
  timeout: _zod.z.number().optional()
});
exports.OsqueryParamsCamelCase = OsqueryParamsCamelCase;
const OsqueryResponseAction = _zod.z.object({
  action_type_id: _zod.z.literal('.osquery'),
  params: OsqueryParams
});
exports.OsqueryResponseAction = OsqueryResponseAction;
const RuleResponseOsqueryAction = _zod.z.object({
  actionTypeId: _zod.z.literal('.osquery'),
  params: OsqueryParamsCamelCase
});
exports.RuleResponseOsqueryAction = RuleResponseOsqueryAction;
const EndpointParams = _zod.z.object({
  command: _zod.z.literal('isolate'),
  comment: _zod.z.string().optional()
});
exports.EndpointParams = EndpointParams;
const EndpointResponseAction = _zod.z.object({
  action_type_id: _zod.z.literal('.endpoint'),
  params: EndpointParams
});
exports.EndpointResponseAction = EndpointResponseAction;
const RuleResponseEndpointAction = _zod.z.object({
  actionTypeId: _zod.z.literal('.endpoint'),
  params: EndpointParams
});
exports.RuleResponseEndpointAction = RuleResponseEndpointAction;
const ResponseAction = _zod.z.union([OsqueryResponseAction, EndpointResponseAction]);
exports.ResponseAction = ResponseAction;
const RuleResponseAction = _zod.z.union([RuleResponseOsqueryAction, RuleResponseEndpointAction]);
exports.RuleResponseAction = RuleResponseAction;