...
class ConfigService {
  ...
  constructor(rawConfigProvider, env, logger) {
    ...
    this.config$ = (0, _rxjs.combineLatest)([this.rawConfigProvider.getConfig$(), this.deprecations, this.overrides$]).pipe((0, _operators.map)(([rawConfig, deprecations, overrides]) => {
  ...
  getValidatedConfigAtPath$(path, {
    ...
    return this.config$.pipe((0, _operators.map)(config => config.get(path)), ignoreUnchanged ? (0, _operators.distinctUntilChanged)(_lodash.isEqual) : _rxjs.identity, (0, _operators.map)(config => this.validateAtPath(path, config)));
  }
  markAsHandled(path) {
