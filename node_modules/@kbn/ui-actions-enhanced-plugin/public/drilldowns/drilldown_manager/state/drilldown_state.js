...
class DrilldownState {
  ...
  constructor({
    ...
    this.nameError$ = this.name$.pipe((0, _operators.map)(currentName => {
      if (!currentName) return 'NAME_EMPTY';
      return undefined;
    }));
    this.triggersError$ = this.triggers$.pipe((0, _operators.map)(currentTriggers => {
      if (!currentTriggers.length) return 'NO_TRIGGERS_SELECTED';
      return undefined;
    }));
    this.configError$ = this.config$.pipe((0, _operators.map)(conf => {
      if (!this.factory.isConfigValid(conf, this.getFactoryContext())) return 'INVALID_CONFIG';
      return undefined;
    }));
    this.error$ = (0, _rxjs.combineLatest)([this.nameError$, this.triggersError$, this.configError$]).pipe((0, _operators.map)(([nameError, configError, triggersError]) => nameError || triggersError || configError || undefined));
  }
  getFactoryContext() {
    return {
      ...this.placeContext,
