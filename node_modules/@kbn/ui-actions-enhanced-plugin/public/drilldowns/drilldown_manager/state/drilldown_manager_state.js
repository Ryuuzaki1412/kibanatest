...
class DrilldownManagerState {
  ...
  constructor(deps) {
    ...
    deps.dynamicActionManager.state.state$.pipe((0, _operators.map)(state => state.events.map(this.mapEventToDrilldownItem))).subscribe(this.events$);
    let {
      initialRoute = ''
    } = deps;
    ...
    this.route$.pipe((0, _operators.map)(() => this.getActiveActionFactory())).subscribe(this.actionFactory$);
  }
