...
const getCertsRequestBody = ({
  ...
  const searchRequest = (0, _es_search.createEsQuery)({
    body: {
      from: (pageIndex !== null && pageIndex !== void 0 ? pageIndex : 0) * size,
      size,
      ...
      query: {
        bool: {
          ...
          filter: [_client_defaults.FINAL_SUMMARY_FILTER, _client_defaults.EXCLUDE_RUN_ONCE_FILTER, ...(filters ? [filters] : []), ...(monitorIds && monitorIds.length > 0 ? [{
            terms: {
      ...
      _source: ['@timestamp', 'config_id', 'monitor.id', 'monitor.name', 'monitor.type', 'url.full', 'observer.geo.name', 'tls.server.x509.issuer.common_name', 'tls.server.x509.subject.common_name', 'tls.server.hash.sha1', 'tls.server.hash.sha256', 'tls.server.x509.not_after', 'tls.server.x509.not_before'],
      collapse: {
        field: 'tls.server.hash.sha256',
        inner_hits: {
          _source: {
