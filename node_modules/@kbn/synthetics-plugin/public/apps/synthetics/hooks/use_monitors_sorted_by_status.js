...
var _use_location_names = require("./use_location_names");
...
function useMonitorsSortedByStatus() {
  ...
  const locationNames = (0, _use_location_names.useLocationNames)();
  const monitorsSortedByStatus = (0, _react.useMemo)(() => {
    ...
    monitors.forEach(monitor => {
      ...
      } else if (monitor.configId in downMonitorMap && downMonitorMap[monitor.configId].includes(monitorLocation)) {
        orderedDownMonitors.push(monitor);
      } else if (pendingConfigs !== null && pendingConfigs !== void 0 && pendingConfigs[`${monitor.configId}-${locationNames[monitor.location.id]}`]) {
