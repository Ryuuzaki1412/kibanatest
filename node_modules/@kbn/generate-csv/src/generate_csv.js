...
class CsvGenerator {
   ...
   */
  logResults(clientDetails, results) {
  ...
  async doSearch(searchSource, settings, searchAfter) {
    ...
    this.logger.debug(`Executing search request with PIT ID: [${this.formatPit(pitId)}]` + (searchAfter ? ` search_after: [${searchAfter}]` : ''));
  ...
  async generateData() {
    ...
    try {
      do {
        var _trackedTotal$value2, _results$pit_id, _hits, _this$job$columns2;
        if (this.cancellationToken.isCancelled()) {
          break;
        }
        ...
        const currentTotal = (_trackedTotal$value2 = trackedTotal === null || trackedTotal === void 0 ? void 0 : trackedTotal.value) !== null && _trackedTotal$value2 !== void 0 ? _trackedTotal$value2 : total;
        if (first) {
        ...
        searchAfter = (_hits = hits[hits.length - 1]) === null || _hits === void 0 ? void 0 : _hits.sort;
    ...
    try {
      ...
      this.logger.error(err);
      warnings.push(_i18n_texts.i18nTexts.csvUnableToClosePit());
    }
  ...
  formatPit(pitId) {
    ...
    return (pitId === null || pitId === void 0 ? void 0 : pitId.substring(0, 12)) + `[${byteSize} bytes]`;
  }
}
