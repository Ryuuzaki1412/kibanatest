...
const logPluginsStatusChanges = ({
  ...
  const throttled$ = buffer.asObservable().pipe((0, _operators.takeUntil)(stop$), (0, _operators.bufferTime)(maxMessagesPerPluginPerInterval), (0, _operators.map)(statuses => {
    const aggregated =
  ...
  const direct$ = plugins$.pipe((0, _operators.startWith)({}),
  // consider all plugins unavailable by default
  (0, _operators.takeUntil)(stop$), (0, _operators.pairwise)(), (0, _operators.map)(([oldStatus, newStatus]) => getPluginUpdates(oldStatus, newStatus)), (0, _operators.concatAll)(), (0, _operators.filter)(pluginStatus => {
    const now = Date.now();
