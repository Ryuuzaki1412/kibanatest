...
const logCoreStatusChanges = ({
  ...
  const throttled$ = buffer.asObservable().pipe((0, _operators.takeUntil)(stop$), (0, _operators.bufferTime)(maxMessagesPerServicePerInterval), (0, _operators.map)(statuses => {
    const aggregated =
  ...
  const direct$ = core$.pipe((0, _operators.startWith)(undefined),
  // consider all services unavailable by default
  (0, _operators.takeUntil)(stop$), (0, _operators.pairwise)(), (0, _operators.map)(([previous, current]) => getServiceUpdates({
    previous,
    current: current
  })), (0, _operators.concatAll)(), (0, _operators.filter)(serviceStatus => {
    const now = Date.now();
