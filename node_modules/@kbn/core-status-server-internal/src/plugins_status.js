...
class PluginsStatusService {
  ...
  constructor(deps, statusTimeoutMs = STATUS_TIMEOUT_MS) {
    ...
    this.coreSubscription = deps.core$.pipe((0, _operators.debounceTime)(10), (0, _operators.tap)(coreStatus => this.coreStatus = coreStatus), (0, _operators.map)(serviceStatuses => (0, _get_summary_status.getSummaryStatus)({
      serviceStatuses
    })),
    // no need to recalculate plugins statuses if core status hasn't changed
    (0, _operators.distinctUntilChanged)((previous, current) => previous.level === current.level)).subscribe(derivedCoreStatus => {
  ...
  set(plugin, status$) {
    ...
    const firstEmissionTimeout$ = this.start$.pipe((0, _operators.delay)(this.statusTimeoutMs), (0, _operators.map)(() => ({
      level: _coreStatusCommon.ServiceStatusLevels.unavailable,
      summary: `Status check timed out after ${this.statusTimeoutMs < 1000 ? `${this.statusTimeoutMs}ms` : `${this.statusTimeoutMs / 1000}s`}`
    })), (0, _operators.takeUntil)(status$));
    this.reportedStatusSubscriptions[plugin] = (0, _rxjs.merge)(firstEmissionTimeout$, status$).pipe((0, _operators.distinctUntilChanged)()).subscribe(status => {
      const {
        levelChanged,
        summaryChanged
