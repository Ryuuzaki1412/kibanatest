...
class MapApp extends _react.default.Component {
  ...
  componentDidMount() {
    ...
    this._autoRefreshSubscription = (0, _kibana_services.getTimeFilter)().getAutoRefreshFetch$().pipe((0, _operators.tap)(() => {
      this.props.setQuery({
        forceRefresh: true
      });
    }), (0, _operators.switchMap)(done => (0, _wait_until_time_layers_load.waitUntilTimeLayersLoad$)(this.props.savedMap.getStore()).pipe((0, _operators.finalize)(done)))).subscribe();
  ...
  _getInitialTime(serializedMapState) {
    if (this._initialTimeFromUrl) {
      return this._initialTimeFromUrl;
    }
    return !this.props.savedMap.hasSaveAndReturnConfig() && serializedMapState !== null && serializedMapState !== void 0 && serializedMapState.timeFilters ? serializedMapState.timeFilters : (0, _kibana_services.getTimeFilter)().getTime();
  }
  _initMapAndLayerSettings(serializedMapState) {
    const globalState = this._getGlobalState();
    const savedObjectFilters = serializedMapState !== null && serializedMapState !== void 0 && serializedMapState.filters ? serializedMapState.filters : [];
