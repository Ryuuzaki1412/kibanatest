...
function isApmIndex(indexPatternTitle) {
  return _observability.APM_INDEX_PATTERN_TITLE.split(',').map(pattern => {
    return (0, _minimatch.default)(indexPatternTitle, pattern);
  }).some(Boolean);
}
function getSourceField(indexPatternTitle) {
  return isApmIndex(indexPatternTitle) ? 'client.geo.location' : 'source.geo.location';
}
function getDestinationField(indexPatternTitle) {
...
function createSourceLayerDescriptor(indexPatternId, indexPatternTitle, parentId) {
  const sourceDescriptor = _es_search_source.ESSearchSource.createDescriptor({
    ...
    tooltipProperties: isApmIndex(indexPatternTitle) ? ['host.name', 'client.ip', 'client.domain', 'client.geo.country_iso_code', 'client.as.organization.name'] : ['host.name', 'source.ip', 'source.domain', 'source.geo.country_iso_code', 'source.as.organization.name']
  });
...
function createDestinationLayerDescriptor(indexPatternId, indexPatternTitle, parentId) {
  const sourceDescriptor = _es_search_source.ESSearchSource.createDescriptor({
    ...
    tooltipProperties: isApmIndex(indexPatternTitle) ? ['host.name', 'server.ip', 'server.domain', 'server.geo.country_iso_code', 'server.as.organization.name'] : ['host.name', 'destination.ip', 'destination.domain', 'destination.geo.country_iso_code', 'destination.as.organization.name']
  });
  const styleProperties = {
    [_constants.VECTOR_STYLES.FILL_COLOR]: {
      type: _constants.STYLE_TYPE.STATIC,
