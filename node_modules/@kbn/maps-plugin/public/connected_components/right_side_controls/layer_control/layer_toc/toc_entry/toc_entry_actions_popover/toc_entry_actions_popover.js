...
class TOCEntryActionsPopover extends _react.Component {
  ...
  async _loadFeatureEditing() {
    if (!(0, _vector_layer.isVectorLayer)(this.props.layer)) {
      return;
    }
  ...
  async _getIsFeatureEditingEnabled() {
    const vectorLayer = this.props.layer;
    const source = this.props.layer.getSource();
    if (!(source instanceof _es_search_source.ESSearchSource)) {
      return false;
    }
    if (source.getSyncMeta().scalingType === _constants.SCALING_TYPES.CLUSTERS || vectorLayer.isPreviewLayer() || !vectorLayer.isVisible() || vectorLayer.hasJoins()) {
      return false;
    }
    return true;
  }
